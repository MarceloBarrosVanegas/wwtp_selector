...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$\nu$] = Viscosidad cinemática del agua a 24.0°C ($8.8200e-07$ m²/s)
\end{itemize}
...
La longitud teórica resultante (2.9~m) es adecuada para el caudal de diseño y se adopta como longitud de diseño.
...
Para la temperatura actual de 24.0°C, los parámetros de diseño adoptados son:

\begin{table}[H]
\centering
\caption{Parámetros de diseño del reactor UASB (T = 24.0°C)}
\small
...
Altura útil & 3,0--6,0 m & 3.63 m & Van Haandel [4] \\
...
\begin{equation}
V_r = \frac{432.0 \times 0.5000}{2.5} = 86.4 \text{ m}^3
\end{equation}
...
La altura resultante del reactor es 3.63 m, proporcionando un tiempo de retención hidráulico de 4.8 horas, valor adecuado según Sperling \cite{sperling2007} para temperaturas superiores a 22°C.
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$Q$] = Caudal diario (432.0 m³/d)
    \item[$S_0$] = DBO afluente (75 mg/L = 75 $\\times$ 10$^{-3}$ kg/m³)
    \item[$C_v$] = Carga orgánica volumétrica (0.30 kg DBO/m³·d)
\end{itemize}

\begin{equation}
V = \frac{432.0 \times 75 \times 10^{-3}}{0.30} = 108.0 \text{ m}^3
\end{equation}
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$T$] = Temperatura del agua (24.0°C)
\end{itemize}

\begin{equation}
k_T = 0.068 \times 1.035^{(24.0-20)} = 0.0780 \text{ m/h}
\end{equation}
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$S_0$] = DBO afluente (75 mg/L)
    \item[$k_T$] = Constante cinética corregida (0.0780 m/h)
...
\begin{equation}
\frac{S_e}{75} = \exp\left(-\frac{0.0780 \times 3.50}{(1.600)^{0.50}}\right) = 0.806
\end{equation}

Por tanto, la DBO efluente estimada es $S_e = 75 \times 0.806 = 60$ mg/L.
...
Considerando la producción estimada de humus del filtro percolador (4.9 kg SST/d):

\begin{equation}
\text{Carga de sólidos} = \frac{4.9 \text{ kg SST/d}}{27.09 \text{ m}^2} = 0.18 \text{ kg SST/m}^2\text{·d}
\end{equation}
...
La producción de lodos se estima considerando una producción específica típica de 0,10 kg SSV/kg DBO removida. Para la carga aplicada, se estima una producción de 15.12 kg SST/d por línea.
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$M_{SST}$] = Masa de sólidos producidos (15.12 kg SST/d)
    \item[$C_{SST}$] = Concentración de sólidos (20 kg/m³)
\end{itemize}

\begin{equation}
V_{lodo} = \frac{15.12}{20} = 0.756 \text{ m}^3\text{/d}
\end{equation}
...
\begin{equation}
A_{total} = \frac{0.756 \times 15}{0.30} = 37.8 \text{ m}^2
\end{equation}
...
\begin{equation}
A_{bloque} = \frac{A_{total}}{n_{lineas}} = \frac{37.8}{2} = 18.9 \text{ m}^2 \text{ por bloque}
\end{equation}
...
\begin{equation}
A_{celda} = \frac{A_{total}}{n_{celdas}} = \frac{37.8}{2} = 18.9 \text{ m}^2 \text{ por celda}
\end{equation}

Con una relación largo/ancho de 3:1, las dimensiones de cada bloque son 7.5 m de largo por 2.5 m de ancho.
...
\begin{equation}
\rho_S = \frac{M_{SST} \times 365}{A_{total}} = \frac{15.12 \times 365}{37.8} = 146.0 \text{ kg SST/m}^2\text{·año}
\end{equation}
...
Parámetro & Valor \\
\midrule
Área total requerida & 37.8 m² \\
Número de bloques (uno por tren) & 2 \\
Área por bloque & 18.9 m² \\
Dimensiones de cada bloque (L×A) & 7.5 m × 2.5 m \\
...
\midrule
DBO$_5$ (mg/L) & 250.0 & 175.0 & 140.0 & 119.0 & 42.5 \\
DQO (mg/L) & 500.0 & 325.0 & 260.0 & 221.0 & 100.0 \\
SST (mg/L) & 250.0 & 175.0 & 70.0 & 21.0 & 5.0 \\
CF (NMP/100mL) & 10000000 & 7000000 & 5600000 & 5600000 & 2500 \\
\bottomrule
\end{tabular}
\end{table}
...
El área de tratamiento calculada (unidades mas margenes minimos) es de aproximadamente 945 m². Sin embargo, para una operacion adecuada se requieren areas complementarias adicionales.

\textbf{Areas complementarias requeridas}

El area calculada de 945 m$^2$ corresponde unicamente a las unidades de tratamiento con sus margenes minimos de acceso. Sin embargo, para garantizar una operacion segura y eficiente de la planta durante toda su vida util, es necesario prever espacios adicionales para acceso vehicular y peatonal de operarios y visitantes, almacenamiento seguro de productos quimicos y herramientas, realizacion de analisis de control de calidad in-situ, estacionamiento para personal y visitantes, circulacion interna de vehiculos de mantenimiento y retiro de lodos, separacion de limites con zonas verdes para control de olores, y espacios administrativos para control operativo. La siguiente tabla resume las areas complementarias necesarias:
...
Considerando el area de tratamiento (945 m$^2$), amortiguacion (20\%), complementarias operativas y zona verde (15\% del total):
...
\begin{itemize}[noitemsep,leftmargin=2em]
    \item[$A_{tratamiento}$] = 945 m$^2$
    \item[$A_{amortiguacion}$] = 189 m$^2$ (20\%)
    \item[$A_{complementarias}$] = 280--340 m$^2$ (operativas)
\end{itemize}

\begin{equation}
A_{total} = \frac{945 + 189 + 310}{0.85} \approx \mathbf{1600 \text{--} 1900 \text{ m}^2} \approx \mathbf{0.16 \text{--} 0.19 \text{ ha}}
\end{equation}
...
DBO$_5$ afluente & 250.0 mg/L \\
DQO afluente & 500.0 mg/L \\
SST afluente & 250.0 mg/L \\
Temperatura & 24.0 °C \\
\bottomrule
\end{tabular}
\end{table}
...
Desarenador & 0.60 $\times$ 3.0 $\times$ 0.7 & 2 & TRH: 30 s \\
Reactor UASB & D = 5.50, H = 3.6 & 2 & v$_{up}$ = 0.76 m/h \\
Filtro Percolador & D = 6.56, H = 4.3 & 2 & Q$_A$ = 4.00 m$^3$/m$^2\cdot$h \\
...
Lecho de Secado & 7.5 $\times$ 2.5 & 2 & Área: 18.9 m$^2$ \\
\bottomrule
\end{tabular}
\end{table}
...
CF (NMP/100mL) & 10000000 & 7000000 & 5600000 & 5600000 & 2500 \\
\midrule
\textbf{Remocion etapa} & -- & \textbf{70\%} & \textbf{19\%} & \textbf{30\%} & \textbf{83.0\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
DBO$_5$ & 42.5 mg/L & $\leq$ 100 mg/L & \textcolor{green}{\textbf{CUMPLE}} \\
DQO & 100.0 mg/L & $\leq$ 250 mg/L & \textcolor{green}{\textbf{CUMPLE}} \\
SST & 5.0 mg/L & $\leq$ 130 mg/L & \textcolor{green}{\textbf{CUMPLE}} \\
Coliformes fecales & 2500 NMP/100mL & $\leq$ 3,000 NMP/100mL & \textcolor{green}{\textbf{CUMPLE}} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Conclusion:} El sistema dise\~nado cumple satisfactoriamente con todos los limites establecidos en la TULSMA (Tabla 12) para descarga a cuerpo de agua dulce.
...
\midrule
Area de tratamiento & 945 m$^2$ \\
Area de amortiguacion (20\%) & 189 m$^2$ \\
Area complementaria operativa & 310 m$^2$ \\
Zona verde (15\% del total) & 142 m$^2$ \\
\midrule
\textbf{Area total requerida} & \textbf{945--1900 m$^2$ (0.16--0.19 ha)} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Produccion lodos UASB & 15.1 kg SST/d \\
Produccion humus FP + Sed & 0.0 kg SST/d \\
\midrule
\textbf{Total lodos a manejar} & \textbf{15.1 kg SST/d} \\
Area de lechos de secado & 37.8 m$^2$ total (2 bloques de 18.9 m$^2$ cada uno) \\
...
Hipoclorito de sodio (12\%) & 15.7 & L/d \\
...
La PTAR (UASB + Filtro Percolador) para 10.0 L/s\\
