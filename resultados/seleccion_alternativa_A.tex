...
\bottomrule
\multicolumn{2}{l}{\small\textit{$^a$Nota: El TRH de 144 s y velocidad de 0.021 m/s se deben al ancho mínimo constructivo de 0.60 m.}} \\
\multicolumn{2}{l}{\small\textit{Aunque el TRH excede el rango típico (30--60 s), la velocidad real de 0.021 m/s es muy inferior a la velocidad crítica de resuspensión (0.228 m/s),}} \\
...
\begin{equation}
V_{biol} = \frac{432.0 \times 0.4980}{2.5} = 86.1 \text{ m}^3
\end{equation}

Este volumen biológico teórico de 86.1 m³ garantizaría el tiempo de retención hidráulico necesario para que los microorganismos anaerobios degraden la materia orgánica según Van Haandel y Lettinga. Sin embargo, el criterio hidráulico (velocidad ascendente) requiere un área superficial mayor para evitar el arrastre del manto de lodos, lo que resulta en un volumen final adoptado de \textbf{125.6 m³}. Este ajuste incrementa el TRH respecto al mínimo teórico, proporcionando un margen de seguridad adicional en la operación del reactor.
...
\textbf{Nota sobre márgenes operativos:} Los parámetros SOR máximo (1.19 m/h) y velocidad en aberturas GLS (4.20 m/h) cumplen con los límites establecidos pero con márgenes operativos reducidos (próximos a los límites de 1.2 m/h y 4.2 m/h respectivamente). Se recomienda verificar estos parámetros durante la puesta en marcha y monitorear periódicamente la calidad del efluente para detectar posibles arrastres de sólidos.
...
\textbf{Verificación de cumplimiento del objetivo:} El valor calculado de $S_e = 57.6$ mg/L excede el objetivo de diseño interno de $55$ mg/L. El diseño es aceptable porque el efluente final post-sedimentador (49.0 mg/L DBO$_5$) cumple con el límite de la TULSMA ($\leq$100 mg/L) y opera con margen de seguridad respecto a la norma aplicable.
...
Caudal por boquilla:
\begin{equation}
q_{boquilla} = \frac{Q_{brazo}}{N_{boquillas}} = \frac{22.5}{8} = 0.78 \text{ L/s} = 2.81 \text{ m}^3\text{/h}
\end{equation}
...
Dado que el caudal por brazo supera el umbral mínimo de 10.0 m³/h, la rotación hidráulica está garantizada sin necesidad de motor auxiliar. La fuerza de reacción generada por el agua al salir por las boquillas proporciona suficiente par de giro para mantener la rotación continua del distribuidor durante la operación normal.
...
\begin{equation}
Q_{\text{diseno}} = \frac{Q_{aplicado}}{0.50} = \frac{45.0}{0.50} = 90.0 \text{ m}^3\text{/h}
\end{equation}
...
\begin{equation}
\text{Llenado} = \frac{Q_{aplicado}}{Q_{canal}} \times 100 = 8.4\\%
\end{equation}

El llenado calculado es inferior al máximo permitido del 50.0 por ciento, garantizando que el canal operará con superficie libre y permitiendo el paso simultáneo de aire para la ventilación natural del sistema.
...
\begin{equation}
\text{Llenado}_{max} = \frac{Q_{aplicado}}{Q_{canal}} \times 100 = 8.4 \\% \\leq 50 \\%
\end{equation}

El llenado calculado cumple satisfactoriamente con el criterio establecido, operando el canal a menos de la mitad de su capacidad. Esta condición asegura que durante la operación normal y condiciones de pico, el sistema mantendrá superficie libre para el flujo de aire, garantizando la ventilación natural del filtro percolador.
...
\begin{equation}
A_{vent} = A_{sup} \times \frac{1.0}{100} = 30.01 \times 0.01 = 0.3001 \text{ m}^2
\end{equation}
...
\begin{equation}
N_{aperturas} = \lceil\frac{A_{vent}}{A_{apertura}}\rceil = \lceil\frac{0.3001}{0.0600}\rceil = \lceil5.0017\rceil = 6
\end{equation}
...
La carga calculada de 402.5 kg/m² es inferior a la resistencia mínima requerida de 600 kg/m² para la profundidad de 3.5 m adoptada. El medio plástico seleccionado cumple satisfactoriamente con el requisito de resistencia a compresión, garantizando la integridad estructural durante toda la vida útil del filtro.
...
Recirculación & R = 1.5 \\
Tasa hidráulica aplicada & 1.50 m³/m²·h \\
\midrule
\multicolumn{2}{l}{\textit{Distribuidor rotatorio}} \\
Número de brazos & 2 \\
Caudal por brazo & 22.5 m³/h \\
Diámetro de orificio & 22.3 mm \\
\midrule
\multicolumn{2}{l}{\textit{Drenaje inferior (underdrain)}} \\
Caudal de diseño & 90.0 m³/h \\
Capacidad del canal & 537.0 m³/h \\
Llenado del canal & 8.4 \\% \\
\midrule
\multicolumn{2}{l}{\textit{Ventilación natural}} \\
Área de ventilación requerida & 0.30 m² \\
Número de aperturas & 6 \\
Espaciado entre aperturas & 3.24 m \\
Caudal de aire (mínimo) & 1.8 m³/h \\
Caudal de aire (óptimo) & 5.4 m³/h \\
\midrule
...
\midrule
Caudal medio por línea (Q) & 432.0 m³/d & – & – \\
Factor de pico horario ($f_p$) & 2.5 & – & Metcalf \& Eddy \\
Caudal máximo horario ($Q_{max}$) & 1080.0 m³/d & – & – \\
Tasa de desbordamiento superficial (SOR) & 16.0 m³/m²·d & 16--32 m³/m²·d & Metcalf \& Eddy \\
Profundidad lateral & 3.50 m & 3,0--4,5 m & Sperling \\
Tiempo de retención hidráulico (HRT) & 5.2 h & $\geq$ 1,5 h & Metcalf \& Eddy \\
SOR máximo permisible & verificado & $\leq$ 45 m³/m²·d & Metcalf \& Eddy \\
Carga sobre vertedero perimetral & 23.5 m³/m·d & $\leq$ 250 m³/m·d & Metcalf \& Eddy \\
...
\begin{equation}
A_s = \frac{432.0}{16.0} = 27.00 \text{ m}^2
\end{equation}

Correspondiendo a un diámetro de 5.86 m para configuración circular. La profundidad lateral de 3.50 m proporciona un volumen útil de 94.5 m³.
...
De acuerdo con los criterios de Metcalf \& Eddy \cite{metcalf2014} y WEF MOP-8 \cite{wef_mop8_2010}, el sedimentador debe verificarse para condiciones de caudal máximo horario. El factor de pico ($f_p$) de 2.5 representa la relación entre el caudal máximo horario y el caudal medio diario, típico de sistemas de alcantarillado municipal. La verificación establece que:

\begin{equation}
Q_{\max} = f_p \times Q = 2.5 \times 432.0 = 1080.0 \text{ m}^3\text{/d}
\end{equation}

A caudal máximo, la tasa de desbordamiento resulta:

\begin{equation}
SOR_{\max} = \frac{Q_{\max}}{A_s} = \frac{1080.0}{27.00} = 40.0 \text{ m}^3\text{/m}^2\text{·d}
\end{equation}

la tasa de desbordamiento máxima (40.0 m³/m²·d) está dentro del rango operativo seguro ($\leq$ 45 m³/m²·d) con un margen del 11.1%. El margen de seguridad respecto al límite de 45 m³/m²·d es del 11.1 por ciento. Según Metcalf \& Eddy (2014), mantener el SOR máximo por debajo de 45 m³/m²·d asegura que la sedimentación no se vea afectada por las condiciones de pico, preservando la calidad del efluente.
...
SOR (caudal máximo) & 40.0 m³/m²·d \\
HRT (promedio) & 5.2 h \\
HRT (caudal máximo) & 2.1 h \\
Carga sobre vertedero & 23.5 m³/m·d \\
...
\caption{Esquema del sedimentador secundario circular: entrada central del efluente del filtro percolador, zona de clarificación, tolva de lodos, vertedero perimetral y salida de efluente clarificado. Diámetro adoptado: 5.86 m, profundidad lateral: 3.50 m, SOR promedio: 16.0 m³/m²·d.}
...
Humus FP + Sedimentador & 0.99 & 1.98 \\
\midrule
\textbf{Total} & \textbf{8.34} & \textbf{16.68} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$M_{SST}$] = Masa de sólidos producidos TOTAL (16.68 kg SST/d)
    \item[$C_{SST}$] = Concentración de sólidos (20 kg/m³)
\end{itemize}

\begin{equation}
V_{lodo} = \frac{16.68}{20} = 0.834 \text{ m}^3\text{/d}
\end{equation}
...
\begin{equation}
A_{total} = \frac{0.834 \times 15}{0.30} = 41.7 \text{ m}^2
\end{equation}
...
\begin{equation}
A_{bloque} = \frac{A_{total}}{n_{lineas}} = \frac{41.7}{2} = 20.9 \text{ m}^2 \text{ por bloque}
\end{equation}
...
\begin{equation}
A_{celda} = \frac{A_{total}}{n_{celdas}} = \frac{41.7}{2} = 20.9 \text{ m}^2 \text{ por celda}
\end{equation}

Con una relación largo/ancho de 3:1, las dimensiones de cada bloque son 7.9 m de largo por 2.6 m de ancho.
...
\begin{equation}
\rho_S = \frac{M_{SST} \times 365}{A_{total}} = \frac{16.68 \times 365}{41.7} = 146.0 \text{ kg SST/m}^2\text{·año}
\end{equation}
...
Parámetro & Valor \\
\midrule
Área total requerida & 41.7 m² \\
Número de bloques (uno por tren) & 2 \\
Área por bloque & 20.9 m² \\
Dimensiones de cada bloque (L×A) & 7.9 m × 2.6 m \\
...
El área de tratamiento calculada (unidades mas margenes minimos) es de aproximadamente 1079 m². Sin embargo, para una operacion adecuada se requieren areas complementarias adicionales.

\textbf{Areas complementarias requeridas}

El area calculada de 1079 m$^2$ corresponde unicamente a las unidades de tratamiento con sus margenes minimos de acceso. Sin embargo, para garantizar una operacion segura y eficiente de la planta durante toda su vida util, es necesario prever espacios adicionales para acceso vehicular y peatonal de operarios y visitantes, almacenamiento seguro de productos quimicos y herramientas, realizacion de analisis de control de calidad in-situ, estacionamiento para personal y visitantes, circulacion interna de vehiculos de mantenimiento y retiro de lodos, separacion de limites con zonas verdes para control de olores, y espacios administrativos para control operativo. La siguiente tabla resume las areas complementarias necesarias:
...
\midrule
Zona de amortiguacion & Perimetral alrededor de unidades (2-3 m) para acceso y seguridad & 216 m$^2$ (20\%) \\
\addlinespace
Bodega de quimicos & Almacenamiento de hipoclorito y productos quimicos & 13 m$^2$ \\
\addlinespace
Laboratorio/control & Analisis de calidad del agua (DBO, SST, CF, pH) & 19 m$^2$ \\
\addlinespace
Caseta de operacion & Control, panel, escritorio, bano & 16 m$^2$ \\
\addlinespace
Area de lavado & Limpieza de equipos y herramientas & 11 m$^2$ \\
\addlinespace
Estacionamiento & Vehiculos del personal y visitantes (2-3 vehiculos) & 59 m$^2$ \\
\addlinespace
Zona de camiones & Acceso de camiones cisterna/retiro de lodo & 70 m$^2$ \\
\addlinespace
Caminos internas & Circulacion vehicular y peatonal (ancho 3--4 m) & 108 m$^2$ (10\% area) \\
\addlinespace
Acceso principal & Entrada, porton y caseta de guardia & 27 m$^2$ \\
\addlinespace
Bodega general & Herramientas, repuestos, EPP & 19 m$^2$ \\
\addlinespace
Zona carga de lodos & Carga/descarga de lodo deshidratado & 27 m$^2$ \\
\addlinespace
Area verde/buffer & Separacion de limites, revegetacion & 276 m$^2$ (15\% total) \\
\bottomrule
\end{tabular}
\end{table}
...
Considerando el area de tratamiento (1079 m$^2$), amortiguacion (20\%), complementarias operativas y zona verde (15\% del total):
...
\begin{itemize}[noitemsep,leftmargin=2em]
    \item[$A_{tratamiento}$] = 1079 m$^2$
    \item[$A_{amortiguacion}$] = 216 m$^2$ (20\%)
    \item[$A_{complementarias}$] = 270 m$^2$ (25\% operativas estimado)
\end{itemize}

\begin{equation}
A_{total} = \frac{1079 + 216 + 270}{0.85} \approx \mathbf{1841 \text{ m}^2} \approx \mathbf{0.18 \text{ ha}}
\end{equation}

\textbf{Nota:} El area total del predio debe ser de aproximadamente 1841 m$^2$ (0.18 ha) para operacion adecuada con circulacion interna, estacionamiento y zonas verdes.
...
Sedimentador Secundario & D = 5.86, H = 3.5 & 2 & SOR = 16.0 m$^3$/m$^2\cdot$d \\
Desinfeccion (Cloro) & 4.2 $\times$ 1.1 $\times$ 2.3 & 2 & CT = 15 mg$\cdot$min/L \\
Lecho de Secado & 7.9 $\times$ 2.6 & 2 & Área: 20.9 m$^2$ \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\textbf{Eficiencia DBO} & -- & 70\% & 21\% & 15\% & 79.8\% \\
\textbf{Eficiencia DQO} & -- & 65\% & 19\% & 15\% & 75.9\% \\
\textbf{Eficiencia SST} & -- & 70\% & 60\% & 80\% & 97.6\% \\
\textbf{Eficiencia CF} & -- & 30\% & 20\% & 10\% & 100.0\% \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Area de tratamiento & 1079 m$^2$ \\
Area de amortiguacion (20\%) & 216 m$^2$ \\
Area complementaria operativa & 270 m$^2$ (25\% estimado) \\
Zona verde (15\% del total) & 276 m$^2$ \\
\midrule
\textbf{Area total requerida} & \textbf{1841 m$^2$ (0.18 ha)} \\
\bottomrule
\end{tabular}
\end{table}
...
Produccion humus FP + Sed & 0.99 kg SST/d & 1.98 kg SST/d \\
\midrule
\textbf{Total lodos} & \textbf{8.34 kg SST/d} & \textbf{16.68 kg SST/d} \\
Area de lechos de secado & --- & 41.7 m$^2$ (2 bloques) \\
