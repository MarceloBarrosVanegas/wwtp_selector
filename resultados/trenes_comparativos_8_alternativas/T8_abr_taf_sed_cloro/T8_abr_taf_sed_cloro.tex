...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{T8 – Rejillas + Desarenador + ABR-RAP + TAF + Sedimentador Secundario + Cloro + Lecho de Secado}
\fancyhead[R]{\thepage}
...
{\LARGE\bfseries T8 – Rejillas + Desarenador + ABR-RAP + TAF + Sedimentador Secundario + Cloro + Lecho de Secado\par}
\vspace{2cm}
...
Este documento presenta el dimensionamiento detallado del T8 – Rejillas + Desarenador + ABR-RAP + TAF + Sedimentador Secundario + Cloro + Lecho de Secado, disenado para tratar un caudal de 17.31 L/s distribuido en 3 lineas paralelas (5.77 L/s por linea).
...
Para este diseño, se establece un CF objetivo de 3000 NMP/100mL, correspondiente al límite establecido por la TULSMA para vertimiento de aguas residuales tratadas. Con una concentración de entrada de 804 NMP/100mL, la reducción logarítmica requerida es:

\begin{equation}
\text{Log reducción}_{\text{req}} = \log_{10}\left(\frac{CF_{\text{entrada}}}{CF_{\text{objetivo}}}\right) = \log_{10}\left(\frac{804}{3000}\right) = 0.00 \text{ log}
\end{equation}
...
\begin{equation}
CF_{final} = \frac{CF_{inicial}}{10^{\text{Log reduccion}}} = \frac{804}{10^{6.6}} = 0 \text{ NMP/100mL}
\end{equation}
...
Lechos de Secado de Lodos & 3.20 & 9.60 \\
\midrule
\textbf{Total} & \textbf{1.60} & \textbf{4.80} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$M_{SST}$] = Masa de solidos producidos TOTAL (4.80 kg SST/d)
    \item[$C_{SST}$] = Concentracion de solidos (20 kg/m$^3$)
\end{itemize}

\begin{equation}
V_{lodo} = \frac{4.80}{20} = 0.240 \text{ m}^3\text{/d}
\end{equation}
...
\begin{equation}
A_{total} = \frac{0.240 \times 15}{0.30} = 12.0 \text{ m}^2
\end{equation}
...
\begin{equation}
A_{bloque} = \frac{A_{total}}{n_{lineas}} = \frac{12.0}{3} = 4.0 \text{ m}^2 \text{ por bloque}
\end{equation}
...
\begin{equation}
A_{celda} = \frac{A_{total}}{n_{celdas}} = \frac{12.0}{3} = 4.0 \text{ m}^2 \text{ por celda}
\end{equation}

Con una relacion largo/ancho de 3:1, las dimensiones de cada bloque son 3.5 m de largo por 1.2 m de ancho.
...
\begin{equation}
\rho_S = \frac{M_{SST} \times 365}{A_{total}} = \frac{4.80 \times 365}{12.0} = 146.0 \text{ kg SST/m}^2\text{·anio}
\end{equation}
...
\bottomrule
\endlastfoot
Area total requerida & 12.0 m$^2$ \\
Numero de bloques (corresponde al numero de lineas) & 3 \\
Area por bloque & 4.0 m$^2$ \\
Dimensiones de cada bloque (L$\times$A) & 3.5 m $\times$ 1.2 m \\
Numero total de celdas & 3 \\
Tiempo de secado & 15 dias \\
Carga de solidos & 146.0 kg SST/m$^2$·anio \\
Produccion total de lodos & 4.80 kg SST/d \\
\end{longtable}
\endgroup
...
El lecho de secado ha sido dimensionado para tratar 4.80 kg SST/d de lodos generados en el proceso de tratamiento, distribuidos en 3 bloques independientes (uno por linea de tratamiento). Cada bloque opera con un ciclo de 15 dias de secado.
