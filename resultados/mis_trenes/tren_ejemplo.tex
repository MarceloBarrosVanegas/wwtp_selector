...
Este documento presenta el dimensionamiento detallado del Sistema de Tratamiento Anaerobio-Aerobio con ABR/RAP, TAF y Sedimentacion, disenado para tratar un caudal de 17.31 L/s distribuido en 3 lineas paralelas (5.77 L/s por linea).
...
Para este diseño, se establece un CF objetivo de 3000 NMP/100mL, correspondiente al límite establecido por la TULSMA para vertimiento de aguas residuales tratadas. Con una concentración de entrada de 938 NMP/100mL, la reducción logarítmica requerida es:

\begin{equation}
\text{Log reducción}_{\text{req}} = \log_{10}\left(\frac{CF_{\text{entrada}}}{CF_{\text{objetivo}}}\right) = \log_{10}\left(\frac{938}{3000}\right) = 0.00 \text{ log}
\end{equation}
...
\begin{equation}
CF_{final} = \frac{CF_{inicial}}{10^{\text{Log reduccion}}} = \frac{938}{10^{6.6}} = 0 \text{ NMP/100mL}
\end{equation}
...
Lechos de Secado de Lodos & 3.88 & 11.64 \\
\midrule
\textbf{Total} & \textbf{1.94} & \textbf{5.82} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$M_{SST}$] = Masa de solidos producidos TOTAL (5.82 kg SST/d)
    \item[$C_{SST}$] = Concentracion de solidos (20 kg/m$^3$)
\end{itemize}

\begin{equation}
V_{lodo} = \frac{5.82}{20} = 0.291 \text{ m}^3\text{/d}
\end{equation}
...
\begin{equation}
A_{total} = \frac{0.291 \times 15}{0.30} = 14.6 \text{ m}^2
\end{equation}
...
\begin{equation}
A_{bloque} = \frac{A_{total}}{n_{lineas}} = \frac{14.6}{3} = 4.9 \text{ m}^2 \text{ por bloque}
\end{equation}
...
\begin{equation}
A_{celda} = \frac{A_{total}}{n_{celdas}} = \frac{14.6}{3} = 4.9 \text{ m}^2 \text{ por celda}
\end{equation}

Con una relacion largo/ancho de 3:1, las dimensiones de cada bloque son 3.8 m de largo por 1.3 m de ancho.
...
\begin{equation}
\rho_S = \frac{M_{SST} \times 365}{A_{total}} = \frac{5.82 \times 365}{14.6} = 146.0 \text{ kg SST/m}^2\text{·anio}
\end{equation}
...
\bottomrule
\endlastfoot
Area total requerida & 14.6 m$^2$ \\
Numero de bloques (corresponde al numero de lineas) & 3 \\
Area por bloque & 4.9 m$^2$ \\
Dimensiones de cada bloque (L$\times$A) & 3.8 m $\times$ 1.3 m \\
Numero total de celdas & 3 \\
Tiempo de secado & 15 dias \\
Carga de solidos & 146.0 kg SST/m$^2$·anio \\
Produccion total de lodos & 5.82 kg SST/d \\
\end{longtable}
\endgroup
...
El lecho de secado ha sido dimensionado para tratar 5.82 kg SST/d de lodos generados en el proceso de tratamiento, distribuidos en 3 bloques independientes (uno por linea de tratamiento). Cada bloque opera con un ciclo de 15 dias de secado.
...
\midrule
Zona de amortiguación & Perimetral alrededor de unidades (2--3~m) para acceso y seguridad & 370~m$^2$ (20\%) \\
\addlinespace
Bodega de químicos & Almacenamiento de hipoclorito y productos químicos & 22~m$^2$ \\
\addlinespace
Laboratorio/control & Análisis de calidad del agua (DBO, SST, CF, pH) & 33~m$^2$ \\
\addlinespace
Caseta de operación & Control, panel, escritorio, baño & 28~m$^2$ \\
\addlinespace
Área de lavado & Limpieza de equipos y herramientas & 18~m$^2$ \\
\addlinespace
Estacionamiento & Vehículos del personal y visitantes (2--3 vehículos) & 102~m$^2$ \\
\addlinespace
Zona de camiones & Acceso de camiones cisterna/retiro de lodo & 120~m$^2$ \\
\addlinespace
Caminos internas & Circulación vehicular y peatonal (ancho 3--4~m) & 185~m$^2$ (10\% área) \\
\addlinespace
Acceso principal & Entrada, portón y caseta de guardia & 46~m$^2$ \\
\addlinespace
Bodega general & Herramientas, repuestos, EPP & 33~m$^2$ \\
\addlinespace
Zona carga de lodos & Carga/descarga de lodo deshidratado & 46~m$^2$ \\
\addlinespace
Área verde/buffer & Separación de límites, revegetación & 473~m$^2$ (15\% total) \\
\bottomrule
\end{tabular}
\end{table}
...
Considerando el área de tratamiento (1849~m$^2$), amortiguación (20\%), complementarias operativas y zona verde (15\% del total):
...
\begin{itemize}[noitemsep,leftmargin=2em]
    \item[$A_{tratamiento}$] = 1849~m$^2$
    \item[$A_{amortiguación}$] = 370~m$^2$ (20\%)
    \item[$A_{complementarias}$] = 462~m$^2$ (25\% operativas estimado)
\end{itemize}

\begin{equation}
A_{total} = \frac{1849 + 370 + 462}{0.85} \approx \mathbf{3154 \text{ m}^2} \approx \mathbf{0.32 \text{ ha}}
\end{equation}

\textbf{Nota:} El área total del predio debe ser de aproximadamente 3154~m$^2$ (0.32~ha) para operación adecuada con circulación interna, estacionamiento y zonas verdes.
...
Lecho de Secado & 3.8 $\times$ 1.3 & 3 & Área: 4.9 m$^2$ \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
DBO$_5$ (mg/L) & 243.1 & 69.3 (70\%) & 13.9 (80\%) & 11.8 (15\%) & 11.8 \\
DQO (mg/L) & 498.0 & 165.6 (65\%) & 41.4 (75\%) & 35.2 (15\%) & 35.2 \\
SST (mg/L) & 156.0 & 37.8 (70\%) & 9.4 (75\%) & 1.9 (80\%) & 1.9 \\
CF (NMP/100mL) & 3300 & 2085 (30\%) & 1042 (50\%) & 938 (10\%) & 1 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Efluente calculado:} DBO$_5$ = 11.8 mg/L | DQO = 35.2 mg/L | SST = 1.9 mg/L | CF = 1 NMP/100mL
...
\midrule
Area de tratamiento & 1849 m$^2$ \\
Area de amortiguacion (20\%) & 370 m$^2$ \\
Area complementaria operativa & 462 m$^2$ (25\% estimado) \\
Zona verde (15\% del total) & 473 m$^2$ \\
\midrule
\textbf{Area total requerida} & \textbf{3154 m$^2$ (0.32 ha)} \\
\bottomrule
\end{tabular}
\end{table}
...
Lechos de Secado de Lodos & 3.88 kg SST/d & 11.64 kg SST/d \\
\midrule
\textbf{Total lodos} & \textbf{1.94 kg SST/d} & \textbf{5.82 kg SST/d} \\
Area de lechos de secado & --- & 14.6 m$^2$ (3 bloques) \\
