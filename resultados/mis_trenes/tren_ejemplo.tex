...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{Sistema de Tratamiento Anaerobio-Aerobio con ABR/RAP, TAF y Sedimentacion}
\fancyhead[R]{\thepage}
...
{\LARGE\bfseries Sistema de Tratamiento Anaerobio-Aerobio con ABR/RAP, TAF y Sedimentacion\par}
\vspace{2cm}
...
Este documento presenta el dimensionamiento detallado del Sistema de Tratamiento Anaerobio-Aerobio con ABR/RAP, TAF y Sedimentacion, disenado para tratar un caudal de 17.31 L/s distribuido en 2 lineas paralelas (8.65 L/s por linea).
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$Q_{medio}$] = Caudal medio diario de diseño (747.8 m³/d = 31.16 m³/h)
...
\begin{equation}
V_{total} = 747.8 \times 1.000 = 747.8 \text{ m}^3
\end{equation}

\textbf{[Ecuación 2] Volumen por compartimento:}

\begin{equation}
V_{comp} = \frac{V_{total}}{n} = \frac{747.8}{4} = 186.95 \text{ m}^3
\end{equation}
...
\begin{equation}
A_{transversal} = \frac{Q_{medio}}{v_{up, dise\tilde{n}o}} = \frac{31.16}{1.0} = 31.16 \text{ m}^2
\end{equation}
...
\begin{equation}
W = \frac{A_{transversal}}{H} = \frac{31.16}{2.0} = 15.58 \text{ m} \quad \text{(ancho)}
\end{equation}

\begin{equation}
L_{comp} = \frac{V_{comp}}{W \times H} = \frac{186.95}{15.58 \times 2.0} = 6.00 \text{ m} \quad \text{(largo por compartimento)}
\end{equation}
...
\begin{equation}
v_{slot} = \frac{Q_{medio}}{W \times h_{slot}} = \frac{31.160}{15.58 \times 0.40} = 5.00 \text{ m/h}
\end{equation}
...
\begin{equation}
v_{up,max} = \frac{Q_{max}}{W \times H} = \frac{86.310}{15.58 \times 2.0} = 2.77 \text{ m/h}
\end{equation}
...
\begin{equation}
TRH_{calc} = \frac{L_{total} \times W \times H_{util}}{Q_{medio}} \times 24 = \frac{24.00 \times 15.58 \times 2.0}{747.8} \times 24 = 24.0 \text{ h}
\end{equation}
...
\begin{equation}
v_{slot,max} = \frac{Q_{max}}{W \times h_{slot}} = \frac{86.310}{15.58 \times 0.40} = 13.85 \text{ m/h}
\end{equation}
...
\begin{equation}
\frac{H_{util}}{W} = \frac{2.0}{15.58} = 0.13
\end{equation}

Estado V-6: \textbf{NO CUMPLE} ($0.5 \leq 0.13 \leq 2.0$)
...
Valor calculado: $L_{comp} / W = 6.00 / 15.58 = 0.39$

Estado V-7: \textbf{BAJO RANGO} (verificación complementaria)
...
\begin{equation}
COV = \frac{Q_{medio} \,[\text{m}^3/\text{d}] \times DBO_{entrada} \,[\text{g/m}^3] \,/\, 1\,000}{V_{total} \,[\text{m}^3]} = \frac{747.8 \times 231 / 1\,000}{747.8} = 0.23 \text{ kg\,DBO/(m}^3\!\cdot\!\text{d)}
\end{equation}
...
\begin{equation}
V_{zona\,lodos} = L_{total} \times W \times H_{lodos} = 24.00 \times 15.58 \times 0.30 = 112.18 \text{ m}^3
\end{equation}
...
V-6 Relación H/W & \texttimes & 0.13 $\in$ [0.5--2.0] \\
\bottomrule
\end{tabular}
\end{table}
...
Volumen total ($V_{total}$) & 747.8 m³ & Calculado por TRH \\
Volumen por compartimento & 186.95 m³ & Distribución uniforme \\
Área sección transversal & 31.16 m² & Por velocidad ascensional \\
Ancho del reactor ($W$) & 15.58 m & Geometría transversal \\
...
Área en planta total & 373.90 m² & $L_{total} \times W$ \\
\midrule
...
\midrule
Zona de amortiguación & Perimetral alrededor de unidades (2--3~m) para acceso y seguridad & 540~m$^2$ (20\%) \\
\addlinespace
Bodega de químicos & Almacenamiento de hipoclorito y productos químicos & 32~m$^2$ \\
\addlinespace
Laboratorio/control & Análisis de calidad del agua (DBO, SST, CF, pH) & 49~m$^2$ \\
\addlinespace
Caseta de operación & Control, panel, escritorio, baño & 40~m$^2$ \\
\addlinespace
Área de lavado & Limpieza de equipos y herramientas & 27~m$^2$ \\
\addlinespace
Estacionamiento & Vehículos del personal y visitantes (2--3 vehículos) & 148~m$^2$ \\
\addlinespace
Zona de camiones & Acceso de camiones cisterna/retiro de lodo & 175~m$^2$ \\
\addlinespace
Caminos internas & Circulación vehicular y peatonal (ancho 3--4~m) & 270~m$^2$ (10\% área) \\
\addlinespace
Acceso principal & Entrada, portón y caseta de guardia & 67~m$^2$ \\
\addlinespace
Bodega general & Herramientas, repuestos, EPP & 49~m$^2$ \\
\addlinespace
Zona carga de lodos & Carga/descarga de lodo deshidratado & 67~m$^2$ \\
\addlinespace
Área verde/buffer & Separación de límites, revegetación & 691~m$^2$ (15\% total) \\
\bottomrule
\end{tabular}
\end{table}
...
Considerando el área de tratamiento (2699~m$^2$), amortiguación (20\%), complementarias operativas y zona verde (15\% del total):
...
\begin{itemize}[noitemsep,leftmargin=2em]
    \item[$A_{tratamiento}$] = 2699~m$^2$
    \item[$A_{amortiguación}$] = 540~m$^2$ (20\%)
    \item[$A_{complementarias}$] = 675~m$^2$ (25\% operativas estimado)
\end{itemize}

\begin{equation}
A_{total} = \frac{2699 + 540 + 675}{0.85} \approx \mathbf{4604 \text{ m}^2} \approx \mathbf{0.46 \text{ ha}}
\end{equation}

\textbf{Nota:} El área total del predio debe ser de aproximadamente 4604~m$^2$ (0.46~ha) para operación adecuada con circulación interna, estacionamiento y zonas verdes.
...
ABR-RAP & 24.0 $\times$ 15.58 & 2 & v$_{up}$ = 1.00 m/h \\
...
\midrule
Area de tratamiento & 2699 m$^2$ \\
Area de amortiguacion (20\%) & 540 m$^2$ \\
Area complementaria operativa & 675 m$^2$ (25\% estimado) \\
Zona verde (15\% del total) & 691 m$^2$ \\
\midrule
\textbf{Area total requerida} & \textbf{4604 m$^2$ (0.46 ha)} \\
\bottomrule
\end{tabular}
\end{table}
