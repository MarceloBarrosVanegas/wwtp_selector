...
Este documento presenta el dimensionamiento detallado del Sistema de Tratamiento ABR/RAP, TAF,SS, disenado para tratar un caudal de 17.31 L/s distribuido en 2 lineas paralelas (8.65 L/s por linea).
...
El caudal de diseno por linea es 8.7 L/s. La seccion transversal del canal se determina aplicando la ecuacion de continuidad:

\begin{equation}
A_{{canal}} = \frac{Q}{v} = \frac{0.00865}{0.50} = 0.01731 \text{ m}^2
\end{equation}

\textit{Donde:}
\begin{itemize}[noitemsep,leftmargin=2em]
    \item[$A_{{canal}}$] = Seccion transversal del canal (m\textsuperscript{2})
    \item[$Q$] = Caudal de diseno por linea (0.00865 m\textsuperscript{3}/s)
...
\begin{equation}
b = \frac{A_{{canal}}}{h} = \frac{0.01731}{0.40} = 0.043 \text{ m}
\end{equation}

\textit{Donde:}
\begin{itemize}[noitemsep,leftmargin=2em]
    \item[$b$] = Ancho teorico del canal (m)
    \item[$A_{{canal}}$] = Seccion transversal del canal (0.01731 m\textsuperscript{2})
    \item[$h$] = Tirante hidraulico (0.40 m)
\end{itemize}

Este ancho teorico de 0.043 m es inferior al minimo constructivo practico. Se adopta un ancho de 0.60 m como dimension minima constructiva segun criterios de OPS/CEPIS \cite{ops2005} y SENAGUA \cite{senagua2012}, que establecen un ancho minimo de 0.60 m para permitir la operacion adecuada, el acceso para limpieza y la aplicacion de herramientas de mantenimiento. Verificando la velocidad real con este ancho:

\begin{equation}
v_{{real}} = \frac{Q}{A_{{real}}} = \frac{0.00865}{0.60 \times 0.40} = 0.036 \text{ m/s}
\end{equation}

\textit{Donde:}
\begin{itemize}[noitemsep,leftmargin=2em]
    \item[$v_{{real}}$] = Velocidad real verificada con ancho constructivo (m/s)
    \item[$Q$] = Caudal de diseno (0.00865 m\textsuperscript{3}/s)
    \item[$A_{{real}}$] = Seccion real con ancho constructivo adoptado (0.60 $\times$ 0.40 m\textsuperscript{2})
\end{itemize}

La velocidad real de 0.036 m/s es el resultado de adoptar el ancho minimo constructivo. La perdida de carga calculada con esta velocidad real resulta $h_L = 0.0081$ cm, menor al umbral de 15 cm que Metcalf y Eddy \cite{metcalf2014} indican como referencia para sistemas de limpieza mecanica.
...
\begin{equation}
Q_{{max}} = 2.8 \times Q_{{medio}} = 2.8 \times 8.7 = 24.0 \text{ L/s}
\end{equation}

A caudal maximo, la velocidad horizontal resulta:

\begin{equation}
v_{{max}} = \frac{Q_{{max}}}{A_{{real}}} = \frac{23.974/1000}{0.60 \times 0.40} = 0.100 \text{ m/s}
\end{equation}

\textbf{Criterios de verificacion de velocidad} (segun Metcalf y Eddy \cite{metcalf2014}):

La velocidad calculada $v_{{max}} = 0.100$~m/s se evalua segun:
...
Con la velocidad calculada $v_{{max}} = 0.100$~m/s, la unidad se clasifica como \textbf{OPTIMO}. En terminos de cumplimiento, la verificacion de velocidad \textbf{CUMPLE} porque es menor que el limite recomendado de 1.5~m/s para operacion segura.

Respecto a la perdida de carga, el valor maximo calculado es 0.0621~cm. Por tanto, la verificacion de perdida de carga \textbf{CUMPLE}, ya que es menor que el umbral de 15~cm para limpieza manual aceptable.
...
Velocidad real & 0.036 m/s & Ancho minimo constructivo$^a$ & Aplicado \\
Perdida de carga (Qmax) & 0.0621 cm & $<$ 15 cm & Cumple \\
Ancho constructivo & 0.60 m & $\geq$ 0.30 m & Cumple \\
\multicolumn{4}{p{\textwidth}}{\small $^a$ Para caudales pequenos (8.7 L/s) el ancho minimo constructivo (0.60 m) domina sobre el criterio de velocidad.} \\
\end{longtable}
\endgroup
...
\begin{equation}
B = \frac{Q}{v_h \cdot H} = \frac{0.00865}{0.30 \times 0.40} = 0.072 \text{ m}
\end{equation}

\textit{Donde:}
\begin{itemize}[noitemsep,leftmargin=2em]
    \item[$B$] = Ancho te\'orico del canal (m)
    \item[$Q$] = Caudal de dise\~no (0.00865 m³/s)
\end{itemize}
...
\begin{equation}
v_{real} = \frac{Q}{B \cdot H} = \frac{0.00865}{0.60 \times 0.40} = 0.036 \text{ m/s}
\end{equation}

El tiempo de retenci\'on real resulta:

\begin{equation}
t_r = \frac{L}{v_{real}} = \frac{3.0}{0.036} = 83.2 \text{ s}
\end{equation}
...
\begin{equation}
v_{h,max} = \frac{Q_{max}}{B \cdot H} = \frac{23.974/1000}{0.60 \times 0.40} = 0.100 \text{ m/s}
\end{equation}

El tiempo de retenci\'on a caudal m\'aximo:

\begin{equation}
t_{r,max} = \frac{L}{v_{h,max}} = \frac{3.0}{0.100} = 30.0 \text{ s}
\end{equation}

\textbf{Criterios de verificaci\'on de resuspensi\'on} (Camp-Shields, Metcalf y Eddy \cite{metcalf2014}):

La velocidad calculada $v_{h,max} = 0.100$~m/s se eval\'ua seg\'un (l\'imite: $v_c \times 90\% = 0.205$~m/s):
...
Con la velocidad calculada $v_{h,max} = 0.100$~m/s, la unidad se clasifica como \textbf{SEGURIDAD}. En terminos de cumplimiento, la verificacion de resuspension \textbf{CUMPLE} porque es menor que el limite de resuspension con factor de seguridad (0.205~m/s), por lo que no se produce resuspension de la arena depositada.
...
Velocidad horizontal real & 0.036 m/s \\
Velocidad cr\'itica resuspensi\'on & 0.228 m/s (Camp-Shields) \\
Tiempo retenci\'on real & 83.2 s \\
\multicolumn{2}{p{\textwidth}}{\small\textit{$^a$Nota: El TRH de 83~s y velocidad de 0.036~m/s se deben al ancho m\'inimo constructivo de 0.60~m. }} \\
\end{longtable}
\endgroup
...
Número de módulos paralelos & 4 & 1 o más \\
Ancho del módulo ($W$) & 1.90 m & $\leq$ 6.0 m \\
Ancho total de la línea & 9.09 m & --- \\
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$Q_{medio}$] = Caudal medio diario de diseño (747.8 m³/d)
...
\begin{equation}
V_{total} = \frac{747.8 \times 231 / 1\,000}{1.00} = 172.7 \text{ m}^3
\end{equation}

El TRH resultante asociado a este volumen es:

\begin{equation}
TRH = \frac{V_{total}}{Q_{medio}} \times 24 = \frac{172.7}{747.8} \times 24 = 5.5 \text{ h}
\end{equation}

\textbf{[Ecuación 2] Volumen por compartimento:}

\begin{equation}
V_{comp} = \frac{V_{total}}{n} = \frac{172.7}{4} = 43.17 \text{ m}^3
\end{equation}
...
\begin{equation}
W = \sqrt{\frac{V_{total}}{n \times (L_{comp}/W)_{obj} \times H}} = \sqrt{\frac{172.7}{4 \times 1.5 \times 2.0}} = 1.90 \text{ m}
\end{equation}

El ancho teórico con un único módulo resultaría $W = 3.79$~m, superior al límite constructivo máximo de $6.0$~m. Por ello se divide la línea en $4$ módulos ABR paralelos, cada uno de $1.90$~m de ancho, con una separación libre entre ellos de $0.5$~m. El ancho total de la línea resulta $9.09$~m.
...
\begin{equation}
L_{comp} = \frac{V_{comp}}{W \times H} = \frac{43.17}{1.90 \times 2.0} = 2.85 \text{ m} \quad \text{(largo por compartimento)}
\end{equation}

\begin{equation}
L_{total} = n \times L_{comp} = 4 \times 2.85 = 11.38 \text{ m} \quad \text{(largo total del módulo)}
\end{equation}

La velocidad ascensional real del módulo se verifica \textit{a posteriori} con el caudal que le corresponde:

\begin{equation}
v_{up,calc} = \frac{Q_{módulo}}{W \times H} = \frac{31.16 / 4}{1.90 \times 2.0} = 2.05 \text{ m/h}
\end{equation}
...
\begin{equation}
v_{slot} = \frac{Q_{módulo}}{W \times h_{slot}} = \frac{31.160 / 4}{1.90 \times 0.40} = 10.25 \text{ m/h}
\end{equation}
...
\begin{equation}
Q_{max} = f_p \times Q_{medio} = 2.8 \times 31.160 = 86.310 \text{ m}^3\text{/h}
\end{equation}

Con este caudal, la velocidad ascensional en pico resulta:

\begin{equation}
v_{up,max} = \frac{Q_{max}}{W \times H} = \frac{86.310}{1.90 \times 2.0} = 5.69 \text{ m/h}
\end{equation}
...
Estado V-2: \textbf{NO CUMPLE} (5.69~m/h $>$ 2.0~m/h)
...
\begin{equation}
TRH_{calc} = \frac{L_{total} \times W \times H_{util}}{Q_{medio}} \times 24 = \frac{11.38 \times 1.90 \times 2.0}{747.8} \times 24 = 5.5 \text{ h}
\end{equation}

Criterio: $TRH_{calc} \geq 6$~h

Estado V-3: \textbf{CUMPLE} (5.5~h $\geq$ 6~h)
...
Valor calculado: $L_{comp} = 2.85$~m

Estado V-4: \textbf{CUMPLE} ($L_{comp} = 2.85$~m $\geq$ 2.0~m)
...
Valor calculado: $W = 1.90$~m (con $4$ módulo(s) paralelo(s)).

Estado V-ancho: \textbf{CUMPLE} (1.90~m $\leq$ 6.0~m)
...
Valor calculado: $L_{total} = 11.38$~m (con $4$ módulo(s) paralelo(s)).

Estado V-largo: \textbf{CUMPLE} (11.38~m $\leq$ 12.0~m)
...
\begin{equation}
v_{slot,max} = \frac{Q_{max,módulo}}{W \times h_{slot}} = \frac{86.310 / 4}{1.90 \times 0.40} = 28.39 \text{ m/h}
\end{equation}

Criterio: $v_{slot} \leq 1.0$~m/h (caudal medio)

Estado V-5: \textbf{NO CUMPLE} (10.25~m/h $>$ 1.0~m/h)
...
\begin{equation}
\frac{H_{util}}{W} = \frac{2.0}{1.90} = 1.05
\end{equation}

Estado V-6: \textbf{CUMPLE} ($0.5 \leq 1.05 \leq 2.0$)
...
\begin{equation}
COV = \frac{Q_{medio} \,[\text{m}^3/\text{d}] \times DBO_{entrada} \,[\text{g/m}^3] \,/\, 1\,000}{V_{total} \,[\text{m}^3]} = \frac{747.8 \times 231 / 1\,000}{172.7} = 1.00 \text{ kg\,DBO/(m}^3\!\cdot\!\text{d)}
\end{equation}
...
Valor calculado: $L_{comp} / W = 2.85 / 1.90 = 1.50$
...
\begin{equation}
DBO_{rem} = \eta_{ref} \times DBO_{entrada} \times \frac{Q_{medio}}{1\,000} = 0.75 \times 231 \times \frac{747.8}{1\,000} = 129.52 \text{ kg\,DBO/d}
\end{equation}

\begin{equation}
P_{lodos} = 0.10 \times 129.52 = 12.95 \text{ kg\,SSV/d}
\end{equation}

Adoptando una densidad de lodos húmedos anaerobios espesados de $\rho = 1\,040$~kg/m³, el volumen acumulado anualmente es:

\begin{equation}
V_{lodos,año} = \frac{P_{lodos} \times 365}{\rho} = \frac{12.95 \times 365}{1\,040} = 4.55 \text{ m}^3\text{/año}
\end{equation}

El volumen disponible en la zona de lodos del reactor es:

\begin{equation}
V_{zona\,lodos} = L_{total} \times W \times H_{lodos} = 11.38 \times 1.90 \times 0.30 = 6.49 \text{ m}^3
\end{equation}

Esto implica una frecuencia indicativa de purga de lodos de aproximadamente \textbf{14 meses}, asumiendo extracción cuando la zona de acumulación alcanza el 80\,\% de su capacidad. Este valor es una estimación de orden de magnitud y deberá ajustarse en la puesta en marcha del sistema en función de las mediciones reales de producción de lodos.
...
V-2 v\_up caudal máximo & \texttimes & 5.69 m/h $\leq$ 2.0 m/h \\
V-3 TRH verificado & \checkmark & 5.5 h $\geq$ 6 h \\
V-4 Largo mínimo compartimento & \checkmark & L\_comp = 2.85 m $\geq$ 2.0 m \\
V-5 Velocidad abertura bafle & \texttimes & 10.25 m/h $\leq$ 1.0 m/h \\
V-6 Relación H/W & \checkmark & 1.05 $\in$ [0.5--2.0] \\
V-COV Carga orgánica volumétrica & \checkmark & 1.00 kg DBO/m³·d $\geq$ 0.50 kg DBO/m³·d \\
V-ancho Ancho del módulo & \checkmark & 1.90 m $\leq$ 6.00 m \\
V-largo Largo total del módulo & \checkmark & 11.38 m $\leq$ 12.00 m \\
V-TRHmin TRH mínimo proceso & \texttimes & 5.5 h $\geq$ 6.0 h \\
\bottomrule
\end{tabular}
\end{table}

\textbf{ATENCIÓN:} las siguientes verificaciones NO cumplen y requieren revisión del diseño: V-2 v\_up caudal máximo, V-5 Velocidad abertura bafle, V-TRHmin TRH mínimo proceso.
...
Caudal medio ($Q_{medio}$) & 747.8 m³/d & Caudal por línea de tratamiento \\
Caudal máximo ($Q_{max}$) & 2071.4 m³/d & Factor pico: 2.8 \\
...
Volumen total ($V_{total}$) & 172.7 m³ & Calculado por COV \\
Volumen por compartimento & 43.17 m³ & Distribución uniforme \\
Número de módulos paralelos & 4 & Por línea de tratamiento \\
Área sección transversal & 3.79 m² & Geometría real del módulo \\
Ancho del módulo ($W$) & 1.90 m & $\leq$ 6.0 m \\
Ancho total de la línea & 9.09 m & Incluye separación entre módulos \\
Largo por compartimento & 2.85 m & $L_{comp} \geq H$ verificado \\
Largo total del módulo ($L_{total}$) & 11.38 m & $n \times L_{comp}$ \\
Profundidad total construcción & 2.60 m & $H + H_{lodos} + H_{bordo}$ \\
Área módulo individual & 21.59 m² & $L_{total} \times W_{módulo}$ \\
Área total línea en planta & 103.42 m² & Huella constructiva total \\
\midrule

\multicolumn{3}{l}{\textit{Parámetros hidráulicos:}} \\
Velocidad ascensional (caudal medio) & 2.05 m/h & NO CUMPLE \\
Velocidad ascensional (caudal máximo) & 5.69 m/h & NO CUMPLE \\
TRH calculado & 5.5 h & CUMPLE \\
Carga hidráulica superficial (CHS) & 7.23 m/d & Parámetro informativo \\
\midrule
...
Verificación TRH & CUMPLE & Criterio: $\geq$ 6 h \\
...
\midrule
Caudal promedio diario & $Q$ & 747.8 & m³/d \\
Caudal máximo horario & $Q_{\text{max}}$ & 2071.4 & m³/d \\
Factor de pico & -- & 2.8 & -- \\
...
\begin{equation}
W = Q \cdot S_0 = 747.8\ \text{m}^3\text{/d} \times 57.7\ \text{g/m}^3 \times 10^{-3} = 43.17\ \text{kg DBO/d}
\end{equation}
...
\begin{equation}
Q_T = Q \cdot (1 + R) = 31.16 \times 2.0 = 62.32\ \text{m}^3\text{/h} = 747.8\ \text{m}^3\text{/d}
\end{equation}
...
\begin{equation}
V = \frac{W}{C_v} = \frac{43.17}{1.00} = 43.2\ \text{m}^3
\end{equation}
...
\begin{equation}
A_s = \frac{V}{D} = \frac{43.2}{3.00} = 14.39\ \text{m}^2 \quad \Rightarrow \quad D_{\text{bf}} = \sqrt{\frac{4A_s}{\pi}} = 4.28\ \text{m}
\end{equation}

La carga hidráulica superficial total, considerando el caudal con recirculación, es:

\begin{equation}
\text{CHS} = \frac{Q_T}{A_s} = \frac{62.32}{14.39} = 4.330\ \text{m}^3\text{/m}^2\text{·h}
\end{equation}
...
Para este diseño se utilizan 2 brazos distribuidores, seleccionados en función del diámetro del biofiltro. El caudal total que llega al distribuidor es $Q_{\text{aplicado}} = 62.32$ m³/h, resultando en un caudal por brazo de:

\begin{equation}
Q_{\text{brazo}} = \frac{Q_{\text{aplicado}}}{N_{\text{brazos}}} = \frac{62.32}{2} = 31.16\ \text{m}^3\text{/h} = 8.66\ \text{L/s}
\end{equation}
...
\begin{equation}
L_{\text{brazo}} = \frac{D_{\text{bf}}}{2} = \frac{4.28}{2} = 2.14\ \text{m}
\end{equation}

Cada brazo dispone de 8 boquillas distribuidas uniformemente, con un caudal por boquilla de 1.08 L/s. El diámetro de orificio calculado para una velocidad de salida de 2.0 m/s resulta $d_{\text{orificio}} = 26.2$ mm, valor dentro del rango constructivo estándar para este tipo de distribuidores.
...
\begin{equation}
\text{COS} = \frac{W}{V} = \frac{43.17}{43.2} = 1.000\ \text{kg DBO/m}^3\text{·d}
\end{equation}
...
\begin{equation}
\text{TRH} = \frac{V}{Q_{\text{aplicado}}} = \frac{43.2}{62.32} = 0.69\ \text{h}
\end{equation}
...
\begin{equation}
P_{\text{humus}} = Y \cdot \Delta\text{DBO} = 0.15 \times 24.01 = 3.60\ \text{kg SST/d}
\end{equation}
\captionequation{Producción de humus en el biofiltro TAF}

Donde $Y = 0.15$ kg SST/kg DBO es el coeficiente de producción de humus típico para biofiltros, y $\Delta\text{DBO} = 24.01$ kg/d es la DBO$_5$ removida en el sistema.

El lodo de humus producido en el biofiltro es relativamente estable aeróbicamente y puede ser manejado junto con los lodos del sedimentador secundario. La producción estimada de 3.60 kg SST/d representa una carga adicional de sólidos que debe acumularse o disponerse adecuadamente.
...
Caudal promedio & 747.8 m³/d \\
Caudal máximo horario & 2071.4 m³/d \\
DBO$_5$ afluente & 57.7 mg/L \\
Carga orgánica & 43.17 kg DBO/d \\
\midrule
...
Caudal de recirculación & $Q_R = 747.8$ m³/d \\
Caudal total aplicado & $Q_T = 62.32$ m³/h \\
\midrule
\multicolumn{2}{l}{\textit{Dimensionamiento}} \\
Volumen de medio & 43.2 m³ \\
Área superficial & 14.39 m² \\
Diámetro & 4.28 m \\
...
DBO$_5$ removida & 24.01 kg/d \\
\midrule
\multicolumn{2}{l}{\textit{Sistema de distribución}} \\
Número de brazos & 2 \\
Caudal por brazo & 31.16 m³/h \\
Longitud de brazo & 2.14 m \\
Número de boquillas/brazo & 8 \\
Diámetro de orificio & 26.2 mm \\
\midrule
\multicolumn{2}{l}{\textit{Producción de lodos}} \\
Humus generado & 3.60 kg SST/d \\
...
\begin{itemize}[noitemsep,leftmargin=2em]
    \item Diámetro: 4.28 m
    \item Altura total: 4.00 m
    \item Volumen de medio filtrante: 43.2 m³
    \item Área superficial: 14.39 m²
\end{itemize}
...
La DBO₅ efluente calculada es 25.6 mg/L. El sistema incluye distribución rotatoria con 2 brazos, sistema underdrain para drenaje y ventilación natural, y produce aproximadamente 3.60 kg SST/d de humus que requiere separación en sedimentador secundario.
...
\caption{Esquema del biofiltro TAF -- Ruta B -- Germain (con recirculación). Diámetro: 4.28~m, profundidad de medio: 3.00~m, altura total: 4.00~m, CHS: 4.330~m³/m²·h, eficiencia DBO$_5$: 56\%.}
...
\midrule
Caudal medio por línea (Q) & 747.8 m³/d & – & – \\
Factor de pico horario ($f_p$) & 2.8 & – & Metcalf \& Eddy \\
Caudal máximo horario ($Q_{max}$) & 2071.4 m³/d & – & – \\
...
Carga sobre vertedero\newline perimetral & 30.9 m³/m·d & $\leq$ 250 m³/m·d & Metcalf \& Eddy \\\\
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$Q$] = Caudal diario (747.8 m³/d)
    \item[$SOR$] = Tasa de desbordamiento superficial (16.0 m³/m²·d)
\end{itemize}

\begin{equation}
A_s = \frac{747.8}{16.0} = 46.74 \text{ m}^2
\end{equation}

Correspondiendo a un diámetro de 7.71 m para configuración circular. La profundidad lateral de 3.50 m proporciona un volumen útil de 163.6 m³.
...
\begin{equation}
P = \pi \times D = \pi \times 7.71 = 24.23 \text{ m}
\end{equation}

La carga sobre el vertedero se calcula como:

\begin{equation}
q_{\text{vertedero}} = \frac{Q}{P} = \frac{747.8}{24.23} = 30.9 \text{ m}^3\text{/m·d}
\end{equation}
...
\begin{equation}
P_{\text{humus}} = 0.15 \times \text{DBO}_{\text{removida}} = 3.6 \text{ kg SST/d}
\end{equation}

La tasa de aplicación de sólidos sobre el área del sedimentador resulta:

\begin{equation}
C_{\text{sólidos}} = \frac{P_{\text{humus}}}{A_s} = \frac{3.6}{46.74} = 0.08 \text{ kg SST/m}^2\text{·d}
\end{equation}
...
\begin{equation}
Q_{\max} = f_p \times Q = 2.8 \times 747.8 = 2071.4 \text{ m}^3\text{/d}
\end{equation}

A caudal máximo, la tasa de desbordamiento resulta:

\begin{equation}
SOR_{\max} = \frac{Q_{\max}}{A_s} = \frac{2071.4}{46.74} = 44.3 \text{ m}^3\text{/m}^2\text{·d}
\end{equation}
...
La carga calculada de 30.9 m³/m·d se compara contra el límite establecido por Metcalf & Eddy de 250 m³/m·d para sedimentadores secundarios. Este límite surge de la necesidad de mantener velocidades de aproximación al vertedero suficientemente bajas que no arrastren los sólidos ya sedimentados hacia el efluente. Con la carga calculada, el estado de la verificación es Cumple.
...
Carga sobre vertedero & 30.9 m³/m·d \\
...
\caption{Esquema del sedimentador secundario circular: entrada central del efluente de la etapa biológica, zona de clarificación, tolva de lodos, vertedero perimetral y salida de efluente clarificado. Diámetro adoptado: 7.71 m, profundidad lateral: 3.50 m, SOR promedio: 16.0 m³/m²·d.}
...
El diseño adopta una filosofía de tiempo de contacto base y residual calculado. Primero se dimensiona el tanque de contacto con un TRH base de referencia (30 min), obteniéndose un TRH real de 35.8 min según las dimensiones constructivas adoptadas. Posteriormente, se calcula el residual requerido para alcanzar el CT objetivo con ese TRH real:

\begin{equation}
C_{\text{req}} = \frac{CT_{\text{req}}}{TRH_{\text{real}}} = \frac{0.0}{35.8} = 0.00 \text{ mg/L}
\end{equation}
...
\begin{equation}
C_{\text{adoptado}} = C_{\text{req}} = 0.84 \text{ mg/L}
\end{equation}
...
\begin{equation}
CT_{\text{real}} = C_{\text{adoptado}} \times TRH_{\text{real}} = 0.84 \times 35.8 = 30.0 \text{ mg$\cdot$min/L}
\end{equation}
...
El cloro residual es la concentración que permanece después de satisfacer la demanda y es la que ejerce el efecto desinfectante. El diseño debe garantizar un residual mínimo de 0.8 mg/L al final del tiempo de contacto para asegurar eficacia. La dosis total a aplicar es:
...
Residual adoptado & 0.84 mg/L & $>$0.00 mg/L & Para cumplir CF objetivo \\
Dosis total & 4.3 mg/L & 3--10 mg/L & Metcalf \& Eddy \cite{metcalf2014} \\
TRH base de diseño & 30 min & 15--45 min & Configuración \\
TRH real del tanque & 35.8 min & $\geq$ 30 min & Dimensionado \\
...
\begin{equation}
\text{Dosis total} = \text{Demanda} + \text{Residual} = 3.5 + 0.8 = 4.3 \text{ mg/L}
\end{equation}
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$C$] = Cloro residual (0.8 mg/L)
    \item[$t$] = Tiempo de contacto real en la cámara (35.8 min)
\end{itemize}

\begin{equation}
CT = 0.84 \times 35.8 = 30.0 \text{ mg$\cdot$min/L}
\end{equation}
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$Q$] = Caudal (747.8 m³/d = 0.519 m³/min)
    \item[$t$] = Tiempo de contacto base (30 min)
\end{itemize}

\begin{equation}
V_{min} = 0.519 \times 30 = 15.6 \text{ m}^3
\end{equation}

El volumen minimo por tiempo de contacto es 15.6 m3; la geometria serpentina adoptada entrega 18.6 m3 y un tiempo hidraulico teorico de 35.8 min.
...
\begin{equation}
\text{Consumo Cl}_2 = \frac{D \times Q}{1000} = \frac{4.3 \times 747.8}{1000} = 3.24 \text{ kg Cl}_2\text{/d}
\end{equation}
...
\begin{equation}
\text{Consumo NaOCl} = \frac{\text{Consumo Cl}_2}{[\% \text{ NaOCl}]} = \frac{3.24}{10\%} = 32.4 \text{ kg NaOCl/d}
\end{equation}
...
\begin{equation}
\text{Volumen NaOCl} = \frac{32.4 \text{ kg/d}}{1.10 \text{ kg/L}} = 29.5 \text{ L/d} \approx 885 \text{ L/mes}
\end{equation}

El volumen de almacenamiento en bodega se recomienda para 30 días de operación: 885 L (equivalente a 0.9 m³).
...
\midrule
Consumo Cl$_2$ activo & 3.24 kg/d \\
Concentración NaOCl & 10 \% \\
Consumo NaOCl & 32.4 kg/d \\
Volumen NaOCl & 29.5 L/d (885 L/mes) \\
Almacenamiento (30 d) & 885 L (0.9 m³) \\
\end{longtable}
\endgroup

El sistema de desinfección con hipoclorito de sodio ha sido dimensionado para tratar un caudal de 747.8 m³/d, logrando una reducción de 100.0\% de coliformes fecales. El cumplimiento respecto a los límites de la TULSMA depende de la categoría de uso receptor específica del proyecto, conforme al dictamen técnico presentado en la tabla de verificación anterior.\
...
\midrule
Reactor Anaerobio con Pantallas (ABR/RAP) & 0.15 & 0.30 \\
Biofiltro de Carga Mecanizada e Hidráulica & 1.80 & 3.60 \\
Sedimentador Secundario & 1.80 & 3.60 \\
\midrule
\textbf{Total} & \textbf{3.75} & \textbf{7.50} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$M_{SST}$] = Masa de solidos producidos TOTAL (7.50 kg SST/d)
    \item[$C_{SST}$] = Concentracion de solidos (20 kg/m$^3$)
\end{itemize}

\begin{equation}
V_{lodo} = \frac{7.50}{20} = 0.375 \text{ m}^3\text{/d}
\end{equation}
...
\begin{equation}
A_{total} = \frac{0.375 \times 15}{0.30} = 18.8 \text{ m}^2
\end{equation}
...
\begin{equation}
A_{bloque} = \frac{A_{total}}{n_{lineas}} = \frac{18.8}{2} = 9.4 \text{ m}^2 \text{ por bloque}
\end{equation}
...
\begin{equation}
A_{celda} = \frac{A_{total}}{n_{celdas}} = \frac{18.8}{2} = 9.4 \text{ m}^2 \text{ por celda}
\end{equation}

Con una relacion largo/ancho de 3:1, las dimensiones de cada bloque son 5.3 m de largo por 1.8 m de ancho.
...
\begin{equation}
\rho_S = \frac{M_{SST} \times 365}{A_{total}} = \frac{7.50 \times 365}{18.8} = 146.0 \text{ kg SST/m}^2\text{·anio}
\end{equation}
...
Area por bloque & 9.4 m$^2$ \\
Dimensiones de cada bloque (L$\times$A) & 5.3 m $\times$ 1.8 m \\
Numero total de celdas & 2 \\
Tiempo de secado & 15 dias \\
Carga de solidos & 146.0 kg SST/m$^2$·anio \\
Produccion total de lodos & 7.50 kg SST/d \\
\end{longtable}
\endgroup
...
El lecho de secado ha sido dimensionado para tratar 7.50 kg SST/d de lodos generados en el proceso de tratamiento, distribuidos en 2 bloques independientes (uno por linea de tratamiento). Cada bloque opera con un ciclo de 15 dias de secado.
...
La siguiente figura presenta la disposición espacial de las unidades de tratamiento. El layout muestra 2 líneas paralelas operativas, cada una con capacidad para tratar 8.7~L/s, permitiendo la operación con una sola línea durante mantenimiento o reparaciones. Las dimensiones del área de tratamiento son aproximadamente 54.6~m de ancho por 27.2~m de largo.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figuras/Layout_sistema_de_tratamiento_abr_rap__taf_ss_2lineas.png}
...
\midrule
Zona de amortiguación & Perimetral alrededor de unidades (2--3~m) para acceso y seguridad & 297~m$^2$ (20\%) \\
\addlinespace
Bodega de químicos & Almacenamiento de hipoclorito y productos químicos & 18~m$^2$ \\
\addlinespace
Laboratorio/control & Análisis de calidad del agua (DBO, SST, CF, pH) & 27~m$^2$ \\
\addlinespace
Caseta de operación & Control, panel, escritorio, baño & 22~m$^2$ \\
\addlinespace
Área de lavado & Limpieza de equipos y herramientas & 15~m$^2$ \\
\addlinespace
Estacionamiento & Vehículos del personal y visitantes (2--3 vehículos) & 82~m$^2$ \\
\addlinespace
Zona de camiones & Acceso de camiones cisterna/retiro de lodo & 96~m$^2$ \\
\addlinespace
Caminos internas & Circulación vehicular y peatonal (ancho 3--4~m) & 148~m$^2$ (10\% área) \\
\addlinespace
Acceso principal & Entrada, portón y caseta de guardia & 37~m$^2$ \\
\addlinespace
Bodega general & Herramientas, repuestos, EPP & 27~m$^2$ \\
\addlinespace
Zona carga de lodos & Carga/descarga de lodo deshidratado & 37~m$^2$ \\
\addlinespace
Área verde/buffer & Separación de límites, revegetación & 380~m$^2$ (15\% total) \\
\bottomrule
\end{tabular}
\end{table}
...
Considerando el área de tratamiento (1484~m$^2$), amortiguación (20\%), complementarias operativas y zona verde (15\% del total):
...
\begin{itemize}[noitemsep,leftmargin=2em]
    \item[$A_{tratamiento}$] = 1484~m$^2$
    \item[$A_{amortiguación}$] = 297~m$^2$ (20\%)
    \item[$A_{complementarias}$] = 371~m$^2$ (25\% operativas estimado)
\end{itemize}

\begin{equation}
A_{total} = \frac{1484 + 297 + 371}{0.85} \approx \mathbf{2532 \text{ m}^2} \approx \mathbf{0.25 \text{ ha}}
\end{equation}

\textbf{Nota:} El área total del predio debe ser de aproximadamente 2532~m$^2$ (0.25~ha) para operación adecuada con circulación interna, estacionamiento y zonas verdes.

\newpage



El presente resumen consolida los resultados del dimensionamiento de la Planta de Tratamiento de Aguas Residuales (PTAR) para un caudal de dise\~no de \textbf{17.3 L/s} (8.7 L/s por linea).
...
Caudal por línea (Q$_{linea}$) & 8.7 L/s (747.8 m$^3$/d) \\
...
Desarenador & 0.60 $\times$ 3.0 $\times$ 0.7 & 2 & TRH: 83~s (referencia: 30--60~s) \\
ABR-RAP & 11.4 $\times$ 1.90 & 2 & v$_{up}$ = 2.05 m/h \\
TAF & D = 4.28, H = 4.0 & 2 & CHS = 4.33 m$^3$/m$^2$·h \\
Sedimentador Secundario & D = 7.71, H = 3.5 & 2 & SOR = 16.0 m$^3$/m$^2\cdot$d \\
Desinfeccion (Cloro) & 3.0 $\times$ 3.1 $\times$ 2.3 & 2 & CT = 30 mg$\cdot$min/L \\
Lecho de Secado & 5.3 $\times$ 1.8 & 2 & Área: 9.4 m$^2$ \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Area de tratamiento & 1484 m$^2$ \\
Area de amortiguacion (20\%) & 297 m$^2$ \\
Area complementaria operativa & 371 m$^2$ (25\% estimado) \\
Zona verde (15\% del total) & 380 m$^2$ \\
\midrule
\textbf{Area total requerida} & \textbf{2532 m$^2$ (0.25 ha)} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Reactor Anaerobio con Pantallas (ABR/RAP) & 0.15 kg SST/d & 0.30 kg SST/d \\
Biofiltro de Carga Mecanizada e Hidráulica & 1.80 kg SST/d & 3.60 kg SST/d \\
Sedimentador Secundario & 1.80 kg SST/d & 3.60 kg SST/d \\
\midrule
\textbf{Total lodos} & \textbf{3.75 kg SST/d} & \textbf{7.50 kg SST/d} \\
Area de lechos de secado & --- & 18.8 m$^2$ (2 bloques) \\
...
Hipoclorito de sodio (10\%) & 29.5 & L/d \\
Consumo hipoclorito anual & 11 & m$^3$/a\~no \\
\bottomrule
\end{tabular}
\end{table}
