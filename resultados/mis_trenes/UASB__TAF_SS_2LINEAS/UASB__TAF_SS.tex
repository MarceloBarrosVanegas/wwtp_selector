...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{Sistema de Tratamiento UASB, TAF,SS}
\fancyhead[R]{\thepage}
...
{\LARGE\bfseries Sistema de Tratamiento UASB, TAF,SS\par}
\vspace{2cm}
...
Este documento presenta el dimensionamiento detallado del Sistema de Tratamiento UASB, TAF,SS, disenado para tratar un caudal de 17.31 L/s distribuido en 2 lineas paralelas (8.65 L/s por linea).
...
Canal de Desbaste con Rejillas $\rightarrow$ Desarenador de Flujo Horizontal $\rightarrow$ Reactor Anaerobio de Flujo Ascendente con Manto de Lodos (UASB) $\rightarrow$ Biofiltro de Carga Mecanizada e Hidráulica $\rightarrow$ Sedimentador Secundario $\rightarrow$ Sistema de Desinfeccion con Hipoclorito de Sodio $\rightarrow$ Lechos de Secado de Lodos
...
\begin{equation}
V_{biol} = \frac{747.8 \times 0.4980}{2.5} = 149.0 \text{ m}^3
\end{equation}

Este volumen biológico teórico de 149.0 m³ garantizaría el tiempo de retención hidráulico necesario para que los microorganismos anaerobios degraden la materia orgánica según Van Haandel y Lettinga. Sin embargo, el criterio hidráulico (velocidad ascendente) requiere un área superficial mayor para evitar el arrastre del manto de lodos, lo que resulta en un volumen final adoptado de \textbf{240.0 m³}. Este ajuste incrementa el TRH respecto al mínimo teórico, proporcionando un margen de seguridad adicional en la operación del reactor.
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$Q$] = Caudal por línea (31.16 m³/h)
    \item[$v_{up}$] = Velocidad ascendente adoptada (0.39 m/h)
\end{itemize}

\begin{equation}
A_s = \frac{31.16}{0.39} = 79.99 \text{ m}^2
\end{equation}
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$A_s$] = Área superficial calculada (79.99 m²)
    \item[$\pi$] = 3,14159
\end{itemize}

\begin{equation}
D = \sqrt{\frac{4 \times 79.99}{\pi}} = 10.09 \text{ m}
\end{equation}
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$Q_d$] = Caudal diario (747.8 m³/d)
    \item[$S_0$] = Concentración de DQO afluente (0.498 kg/m³)
    \item[$E$] = Eficiencia de remoción (0.65)
\end{itemize}

\begin{equation}
V_{CH_4} = (747.8 \times 0.498 \times 0.65) \times 0.35 = 84.7 \text{ m}^3 \text{ CH}_4\text{/d}
\end{equation}
...
\begin{equation}
A_{sed} = 0.90 \times A_{sup} = 0.90 \times 79.99 = 71.99 \text{ m}^2
\end{equation}

El volumen del sedimentador se calcula con la altura adoptada de 1.6~m:

\begin{equation}
V_{sed} = A_{sed} \times H_{sed} = 71.99 \times 1.6 = 115.19 \text{ m}^3
\end{equation}

El tiempo de retención hidráulico en el sedimentador resulta:

\begin{equation}
t_{sed} = \frac{V_{sed}}{Q} = \frac{115.19}{31.16} = 3.70 \text{ h}
\end{equation}
...
\begin{equation}
A_{ab} = \frac{31.16}{1.52} = 20.55 \text{ m}^2
\end{equation}
...
\begin{equation}
N = \frac{A}{A_{inf}} = \frac{79.99}{2.50} = 32 \text{ puntos}
\end{equation}
...
\begin{equation}
q = \frac{Q}{N} = \frac{0.009 \times 1000}{32} = 0.270 \text{ L/s}
\end{equation}
...
\begin{equation}
v_{tubo} = \frac{q}{a_{tubo}} = \frac{0.270 \times 10^{-3}}{\pi \times (50 \times 10^{-3})^2 / 4} = 0.138 \text{ m/s}
\end{equation}
...
\begin{equation}
v_{boca} = \frac{q}{a_{boca}} = \frac{0.270 \times 10^{-3}}{\pi \times (25 \times 10^{-3})^2 / 4} = 0.55 \text{ m/s}
\end{equation}
...
\begin{equation}
v_{up,max} = \frac{Q_{max}}{A_s} = \frac{86.31}{79.99} = 1.08 \text{ m/h}
\end{equation}
...
\begin{equation}
SOR_{medio} = \frac{Q_{medio}}{A_{sed}} = \frac{31.16}{71.99} = 0.43 \text{ m/h}
\end{equation}

Para caudal máximo horario (condición de pico):
\begin{equation}
SOR_{max} = \frac{Q_{max}}{A_{sed}} = \frac{86.31}{71.99} = 1.20 \text{ m/h}
\end{equation}
...
El análisis de cumplimiento de criterios evalúa los valores calculados según los parámetros establecidos por Chernicharo (2007) para sedimentadores en reactores UASB:
     ...
     \textbf{TRH en sedimentador}: El tiempo de retencion de 3.70 h cumple con el minimo de 1.5 h recomendado por Chernicharo para permitir la adecuada separacion gas-liquido-solido.
...
TRH sedimentador & 3.70 h & $\geq$ 1.5 h & Cumple \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Nota sobre márgenes operativos:} Los parámetros críticos de control hidráulico del reactor se mantienen dentro de los límites recomendados por Chernicharo \cite{chernicharo2007}. El SOR máximo es 1.20 m/h frente al límite de 1.2 m/h, con un margen de 0.1%; este criterio controla el riesgo de arrastre de sólidos en la cámara de sedimentación durante caudales pico. La velocidad máxima en aberturas GLS es 4.20 m/h frente al límite de 4.2 m/h, con un margen de 0.0%; este criterio controla el paso del líquido clarificado hacia el compartimiento de sedimentación sin inducir arrastre de lodos. En conjunto, el margen operativo se clasifica como reducido. Por ello, aunque la unidad cumple, se recomienda verificar SOR y velocidad en aberturas durante la puesta en marcha y monitorear periódicamente SST/turbidez del efluente como indicador de posible arrastre.
...
Verificando para caudal máximo ($Q_{max} = 86.31$ m³/h):

\begin{equation}
v_{ab,max} = \frac{Q_{max}}{A_{ab}} = \frac{86.31}{20.55} = 4.20 \text{ m/h} \quad \leq 4.2 \text{ m/h } \text{(Cumple)}
\end{equation}

La velocidad máxima calculada en las aberturas GLS antes del ajuste era 5.96 m/h, superior al límite de 4.2 m/h. Para cumplir, se aumentó el área libre de aberturas a 20.55 m², quedando la velocidad máxima ajustada exactamente en 4.20 m/h. Esta condición indica que el diseño cumple técnicamente pero queda en el límite (margen 0 %). Se recomienda mantener limpias las ranuras del GLS y verificar su condición durante inspecciones de rutina.
...
\midrule
Velocidad en tubería principal& 0.138 m/s & Transporte sin obstrucción ($<$ 0.20 m/s) & Adecuado \\
Velocidad en boca de salida & 0.55 m/s & 0.4--4.0 m/s & Cumple \\
\bottomrule
\end{tabular}
\end{table}

La velocidad en boca de 0.55 m/s cumple con el rango recomendado por Lettinga y Hulshoff Pol para inyeccion adecuada en el lecho de lodo.
...
\midrule
Volumen útil & 240.0 m³ \\
Área superficial & 79.99 m² \\
Tiempo de retención hidráulico & 7.7 h \\
Carga orgánica volumétrica & 2.5 kg DQO/m³·d \\
Biogás producido & 84.7 m$^3$ CH$_4$/d \\
\midrule
...
\caption{Esquema del reactor UASB: distribución del afluente, zonas de reacción, separación gas-líquido-sólido y recolección de biogás. Caudal por línea: 31.16 m³/h, Biogás: 84.7 m³ CH$_4$/d, 32 puntos de distribución.}
...
Carga hidráulica superficial CHS & 0.5--4.0 m³/m²·h & 3.608 m³/m²·h & Proyecto \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{equation}
W = Q \cdot S_0 = 747.8\ \text{m}^3\text{/d} \times 69.3\ \text{g/m}^3 \times 10^{-3} = 51.81\ \text{kg DBO/d}
\end{equation}
...
\begin{equation}
S_0' = \frac{S_0}{1 + R} = \frac{69.3}{1 + 1.0} = 34.6\ \text{mg/L}
\end{equation}
...
\begin{equation}
V = \frac{W}{C_v} = \frac{51.81}{1.00} = 51.8\ \text{m}^3
\end{equation}
...
\begin{equation}
A_s = \frac{V}{D} = \frac{51.8}{3.00} = 17.27\ \text{m}^2 \quad \Rightarrow \quad D_{\text{bf}} = \sqrt{\frac{4A_s}{\pi}} = 4.69\ \text{m}
\end{equation}

La carga hidráulica superficial total, considerando el caudal con recirculación, es:

\begin{equation}
\text{CHS} = \frac{Q_T}{A_s} = \frac{62.32}{17.27} = 3.608\ \text{m}^3\text{/m}^2\text{·h}
\end{equation}
...
\begin{equation}
L_{\text{brazo}} = \frac{D_{\text{bf}}}{2} = \frac{4.69}{2} = 2.34\ \text{m}
\end{equation}
...
\begin{equation}
S_e = S_0' \cdot \exp\!\left(-\frac{k_T \cdot D}{Q_T^n}\right) = 34.6 \times \exp\!\left(-\frac{0.0824 \times 3.00}{(3.608)^{0.5}}\right) = 30.4\ \text{mg/L}
\end{equation}
...
\begin{equation}
E = \frac{S_0 - S_e}{S_0} = \frac{69.3 - 30.4}{69.3} = 0.561 \approx 56.1\%
\end{equation}

La eficiencia calculada de 56.1\% refleja el desempeño esperado del biofiltro con los parámetros de diseño adoptados.

\textbf{Resultado de verificación:} La DBO₅ efluente calculada es 30.4 mg/L.
...
\begin{equation}
\text{COS} = \frac{W}{V} = \frac{51.81}{51.8} = 1.000\ \text{kg DBO/m}^3\text{·d}
\end{equation}
...
\begin{equation}
\text{CHS} = \frac{Q_{\text{aplicado}}}{A_s} = 3.608\ \text{m}^3\text{/m}^2\text{·h} = 86.60\ \text{m}^3\text{/m}^2\text{·d}
\end{equation}
\captionequation{Carga hidráulica superficial del biofiltro}

La CHS calculada de 3.608 m³/m²·h (86.60 m/d) se encuentra dentro del rango operacional recomendado (0.5--4.0 m³/m²·h). Este valor garantiza un riego adecuado del medio y mantiene la biopelícula húmeda y activa.
...
\begin{equation}
P_{\text{humus}} = Y \cdot \Delta\text{DBO} = 0.15 \times 29.07 = 4.36\ \text{kg SST/d}
\end{equation}
\captionequation{Producción de humus en el biofiltro TAF}

Donde $Y = 0.15$ kg SST/kg DBO es el coeficiente de producción de humus típico para biofiltros, y $\Delta\text{DBO} = 29.07$ kg/d es la DBO$_5$ removida en el sistema.

El lodo de humus producido en el biofiltro es relativamente estable aeróbicamente y puede ser manejado junto con los lodos del sedimentador secundario. La producción estimada de 4.36 kg SST/d representa una carga adicional de sólidos que debe acumularse o disponerse adecuadamente.
...
DBO$_5$ afluente & 69.3 mg/L \\
Carga orgánica & 51.81 kg DBO/d \\
\midrule
...
Carga hidráulica superficial (CHS) & 3.608 m³/m²·h \\
Tiempo de retención hidráulico & 0.83 h \\
\midrule
\multicolumn{2}{l}{\textit{Desempeño esperado}} \\
Eficiencia de remoción DBO$_5$ & 56.1\% \\
DBO$_5$ efluente calculada & 30.4 mg/L \\
DBO$_5$ removida & 29.07 kg/d \\
\midrule
\multicolumn{2}{l}{\textit{Sistema de distribución}} \\
Número de brazos & 2 \\
Caudal por brazo & 31.16 m³/h \\
Longitud de brazo & 2.34 m \\
Número de boquillas/brazo & 8 \\
Diámetro de orificio & 26.2 mm \\
\midrule
\multicolumn{2}{l}{\textit{Producción de lodos}} \\
Humus generado & 4.36 kg SST/d \\
SST efluente estimado & 49.5 mg/L \\
\end{longtable}
\endgroup
...
\begin{itemize}[noitemsep,leftmargin=2em]
    \item Diámetro: 4.69 m
    \item Altura total: 4.00 m
    \item Volumen de medio filtrante: 51.8 m³
    \item Área superficial: 17.27 m²
\end{itemize}

El desempeño esperado del sistema, verificado mediante el modelo cinético correspondiente, indica:

\begin{itemize}[noitemsep,leftmargin=2em]
    \item Eficiencia de remoción de DBO$_5$: 56.1\%
    \item DBO$_5$ del efluente: 30.4 mg/L

\end{itemize}

La DBO₅ efluente calculada es 30.4 mg/L. El sistema incluye distribución rotatoria con 2 brazos, sistema underdrain para drenaje y ventilación natural, y produce aproximadamente 4.36 kg SST/d de humus que requiere separación en sedimentador secundario.
...
\caption{Esquema del biofiltro TAF -- Ruta B -- Germain (con recirculación). Diámetro: 4.69~m, profundidad de medio: 3.00~m, altura total: 4.00~m, CHS: 3.608~m³/m²·h, eficiencia DBO$_5$: 56\%.}
...
\begin{equation}
P_{\text{humus}} = 0.15 \times \text{DBO}_{\text{removida}} = 4.4 \text{ kg SST/d}
\end{equation}

La tasa de aplicación de sólidos sobre el área del sedimentador resulta:

\begin{equation}
C_{\text{sólidos}} = \frac{P_{\text{humus}}}{A_s} = \frac{4.4}{46.74} = 0.09 \text{ kg SST/m}^2\text{·d}
\end{equation}
...
La tasa de aplicación de sólidos calculada de 0.09 kg SST/m²·d se compara contra el límite conservador de 100 kg SST/m²·d adoptado para este diseño, basado en criterios de Metcalf & Eddy para sedimentadores secundarios después de procesos biológicos aerobios. El rango operativo típico de referencia es 50--150 kg SST/m²·d. El estado de la verificación es Cumple.
...
\midrule
Reactor Anaerobio de Flujo Ascendente con Manto de Lodos (UASB) & 7.96 & 15.91 \\
Biofiltro de Carga Mecanizada e Hidráulica & 2.18 & 4.36 \\
Sedimentador Secundario & 2.18 & 4.36 \\
\midrule
\textbf{Total} & \textbf{12.32} & \textbf{24.63} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$M_{SST}$] = Masa de solidos producidos TOTAL (24.63 kg SST/d)
    \item[$C_{SST}$] = Concentracion de solidos (20 kg/m$^3$)
\end{itemize}

\begin{equation}
V_{lodo} = \frac{24.63}{20} = 1.232 \text{ m}^3\text{/d}
\end{equation}
...
\begin{equation}
A_{total} = \frac{1.232 \times 15}{0.30} = 61.6 \text{ m}^2
\end{equation}
...
\begin{equation}
A_{bloque} = \frac{A_{total}}{n_{lineas}} = \frac{61.6}{2} = 30.8 \text{ m}^2 \text{ por bloque}
\end{equation}
...
\begin{equation}
A_{celda} = \frac{A_{total}}{n_{celdas}} = \frac{61.6}{2} = 30.8 \text{ m}^2 \text{ por celda}
\end{equation}

Con una relacion largo/ancho de 3:1, las dimensiones de cada bloque son 9.6 m de largo por 3.2 m de ancho.
...
\begin{equation}
\rho_S = \frac{M_{SST} \times 365}{A_{total}} = \frac{24.63 \times 365}{61.6} = 146.0 \text{ kg SST/m}^2\text{·anio}
\end{equation}
...
\bottomrule
\endlastfoot
Area total requerida & 61.6 m$^2$ \\
Numero de bloques (corresponde al numero de lineas) & 2 \\
Area por bloque & 30.8 m$^2$ \\
Dimensiones de cada bloque (L$\times$A) & 9.6 m $\times$ 3.2 m \\
Numero total de celdas & 2 \\
Tiempo de secado & 15 dias \\
Carga de solidos & 146.0 kg SST/m$^2$·anio \\
Produccion total de lodos & 24.63 kg SST/d \\
\end{longtable}
\endgroup
...
El lecho de secado ha sido dimensionado para tratar 24.63 kg SST/d de lodos generados en el proceso de tratamiento, distribuidos en 2 bloques independientes (uno por linea de tratamiento). Cada bloque opera con un ciclo de 15 dias de secado.
...
\midrule
Zona de amortiguación & Perimetral alrededor de unidades (2--3~m) para acceso y seguridad & 330~m$^2$ (20\%) \\
\addlinespace
Bodega de químicos & Almacenamiento de hipoclorito y productos químicos & 20~m$^2$ \\
\addlinespace
Laboratorio/control & Análisis de calidad del agua (DBO, SST, CF, pH) & 30~m$^2$ \\
\addlinespace
Caseta de operación & Control, panel, escritorio, baño & 25~m$^2$ \\
\addlinespace
Área de lavado & Limpieza de equipos y herramientas & 17~m$^2$ \\
\addlinespace
Estacionamiento & Vehículos del personal y visitantes (2--3 vehículos) & 91~m$^2$ \\
\addlinespace
Zona de camiones & Acceso de camiones cisterna/retiro de lodo & 107~m$^2$ \\
\addlinespace
Caminos internas & Circulación vehicular y peatonal (ancho 3--4~m) & 165~m$^2$ (10\% área) \\
\addlinespace
Acceso principal & Entrada, portón y caseta de guardia & 41~m$^2$ \\
\addlinespace
Bodega general & Herramientas, repuestos, EPP & 30~m$^2$ \\
\addlinespace
Zona carga de lodos & Carga/descarga de lodo deshidratado & 41~m$^2$ \\
\addlinespace
Área verde/buffer & Separación de límites, revegetación & 422~m$^2$ (15\% total) \\
\bottomrule
\end{tabular}
\end{table}
...
Considerando el área de tratamiento (1651~m$^2$), amortiguación (20\%), complementarias operativas y zona verde (15\% del total):
...
\begin{itemize}[noitemsep,leftmargin=2em]
    \item[$A_{tratamiento}$] = 1651~m$^2$
    \item[$A_{amortiguación}$] = 330~m$^2$ (20\%)
    \item[$A_{complementarias}$] = 413~m$^2$ (25\% operativas estimado)
\end{itemize}

\begin{equation}
A_{total} = \frac{1651 + 330 + 413}{0.85} \approx \mathbf{2816 \text{ m}^2} \approx \mathbf{0.28 \text{ ha}}
\end{equation}

\textbf{Nota:} El área total del predio debe ser de aproximadamente 2816~m$^2$ (0.28~ha) para operación adecuada con circulación interna, estacionamiento y zonas verdes.
...
Reactor UASB & D = 10.09, H = 6.4 & 2 & v$_{up}$ = 0.39 m/h \\
TAF & D = 4.69, H = 4.0 & 2 & CHS = 3.61 m$^3$/m$^2$·h \\
...
Lecho de Secado & 9.6 $\times$ 3.2 & 2 & Área: 30.8 m$^2$ \\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
\textbf{} & \textbf{Afluente} & \textbf{Post-UASB} & \textbf{Post-TAF} & \textbf{Post-Sed} & \textbf{Efluente} \\
\midrule
DBO$_5$ (mg/L) & 243.1 & 69.3 (70\%) & 30.4 (56\%) & 25.8 (15\%) & 25.8 \\
DQO (mg/L) & 498.0 & 165.6 (65\%) & 81.9 (51\%) & 69.6 (15\%) & 69.6 \\
SST (mg/L) & 156.0 & 37.8 (70\%) & 49.5 (0\%) & 9.9 (80\%) & 9.9 \\
CF (NMP/100mL) & 3300 & 2085 (30\%) & 1668 (20\%) & 1501 (10\%) & 1 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Efluente calculado:} DBO$_5$ = 25.8 mg/L | DQO = 69.6 mg/L | SST = 9.9 mg/L | CF = 1 NMP/100mL
...
\midrule
Area de tratamiento & 1651 m$^2$ \\
Area de amortiguacion (20\%) & 330 m$^2$ \\
Area complementaria operativa & 413 m$^2$ (25\% estimado) \\
Zona verde (15\% del total) & 422 m$^2$ \\
\midrule
\textbf{Area total requerida} & \textbf{2816 m$^2$ (0.28 ha)} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Reactor Anaerobio de Flujo Ascendente con Manto de Lodos (UASB) & 7.96 kg SST/d & 15.91 kg SST/d \\
Biofiltro de Carga Mecanizada e Hidráulica & 2.18 kg SST/d & 4.36 kg SST/d \\
Sedimentador Secundario & 2.18 kg SST/d & 4.36 kg SST/d \\
\midrule
\textbf{Total lodos} & \textbf{12.32 kg SST/d} & \textbf{24.63 kg SST/d} \\
Area de lechos de secado & --- & 61.6 m$^2$ (2 bloques) \\
