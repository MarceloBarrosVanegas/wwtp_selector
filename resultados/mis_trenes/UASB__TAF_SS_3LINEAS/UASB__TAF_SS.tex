...
Este documento presenta el dimensionamiento detallado del Sistema de Tratamiento UASB, TAF,SS, disenado para tratar un caudal de 17.31 L/s distribuido en 3 lineas paralelas (5.77 L/s por linea).
...
\begin{equation}
Q_{max} = f_p \times Q_{medio} = 2.77 \times 498.5 = 1380.9 \text{ m}^3\text{/d} = 57.54 \text{ m}^3\text{/h}
\end{equation}
...
La velocidad máxima calculada en las aberturas GLS antes del ajuste era 5.96 m/h, superior al límite de 4.2 m/h. Para cumplir, se aumentó el área libre de aberturas a 13.70 m², quedando la velocidad máxima ajustada exactamente en 4.20 m/h. Esta condición indica que el diseño cumple técnicamente pero queda en el límite (margen 0 %). Se recomienda mantener limpias las ranuras del GLS y verificar su condición durante inspecciones de rutina.
...
\begin{equation}
W = Q \cdot S_0 = 498.5\ \text{m}^3\text{/d} \times 69.3\ \text{g/m}^3 \times 10^{-3} = 34.54\ \text{kg DBO/d}
\end{equation}
...
\begin{equation}
V = \frac{W}{C_v} = \frac{34.54}{1.00} = 34.5\ \text{m}^3
\end{equation}
...
\begin{equation}
A_s = \frac{V}{D} = \frac{34.5}{3.00} = 11.51\ \text{m}^2 \quad \Rightarrow \quad D_{\text{bf}} = \sqrt{\frac{4A_s}{\pi}} = 3.83\ \text{m}
\end{equation}

La carga hidráulica superficial total, considerando el caudal con recirculación, es:

\begin{equation}
\text{CHS} = \frac{Q_T}{A_s} = \frac{41.54}{11.51} = 3.608\ \text{m}^3\text{/m}^2\text{·h}
\end{equation}
...
\begin{equation}
L_{\text{brazo}} = \frac{D_{\text{bf}}}{2} = \frac{3.83}{2} = 1.91\ \text{m}
\end{equation}
...
\begin{equation}
\text{COS} = \frac{W}{V} = \frac{34.54}{34.5} = 1.000\ \text{kg DBO/m}^3\text{·d}
\end{equation}
...
\begin{equation}
\text{TRH} = \frac{V}{Q_{\text{aplicado}}} = \frac{34.5}{41.54} = 0.83\ \text{h}
\end{equation}
...
\begin{equation}
P_{\text{humus}} = Y \cdot \Delta\text{DBO} = 0.15 \times 19.38 = 2.91\ \text{kg SST/d}
\end{equation}
\captionequation{Producción de humus en el biofiltro TAF}

Donde $Y = 0.15$ kg SST/kg DBO es el coeficiente de producción de humus típico para biofiltros, y $\Delta\text{DBO} = 19.38$ kg/d es la DBO$_5$ removida en el sistema.

El lodo de humus producido en el biofiltro es relativamente estable aeróbicamente y puede ser manejado junto con los lodos del sedimentador secundario. La producción estimada de 2.91 kg SST/d representa una carga adicional de sólidos que debe acumularse o disponerse adecuadamente.
...
Volumen de medio & 34.5 m³ \\
Área superficial & 11.51 m² \\
Diámetro & 3.83 m \\
...
\begin{itemize}[noitemsep,leftmargin=2em]
    \item Diámetro: 3.83 m
    \item Altura total: 4.00 m
    \item Volumen de medio filtrante: 34.5 m³
    \item Área superficial: 11.51 m²
\end{itemize}
...
La DBO₅ efluente calculada es 30.4 mg/L. El sistema incluye distribución rotatoria con 2 brazos, sistema underdrain para drenaje y ventilación natural, y produce aproximadamente 2.91 kg SST/d de humus que requiere separación en sedimentador secundario.
...
\caption{Esquema del biofiltro TAF -- Ruta B -- Germain (con recirculación). Diámetro: 3.83~m, profundidad de medio: 3.00~m, altura total: 4.00~m, CHS: 3.608~m³/m²·h, eficiencia DBO$_5$: 56\%.}
...
\begin{equation}
P_{\text{humus}} = 0.15 \times \text{DBO}_{\text{removida}} = 2.9 \text{ kg SST/d}
\end{equation}

La tasa de aplicación de sólidos sobre el área del sedimentador resulta:

\begin{equation}
C_{\text{sólidos}} = \frac{P_{\text{humus}}}{A_s} = \frac{2.9}{31.16} = 0.09 \text{ kg SST/m}^2\text{·d}
\end{equation}
...
Biofiltro de Carga Mecanizada e Hidráulica & 0.97 & 2.91 \\
Sedimentador Secundario & 0.97 & 2.91 \\
\midrule
\textbf{Total} & \textbf{5.47} & \textbf{16.42} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[noitemsep,leftmargin=2em]
    ...
    \item[$M_{SST}$] = Masa de solidos producidos TOTAL (16.42 kg SST/d)
    \item[$C_{SST}$] = Concentracion de solidos (20 kg/m$^3$)
\end{itemize}

\begin{equation}
V_{lodo} = \frac{16.42}{20} = 0.821 \text{ m}^3\text{/d}
\end{equation}
...
\begin{equation}
A_{total} = \frac{0.821 \times 15}{0.30} = 41.1 \text{ m}^2
\end{equation}
...
\begin{equation}
A_{bloque} = \frac{A_{total}}{n_{lineas}} = \frac{41.1}{3} = 13.7 \text{ m}^2 \text{ por bloque}
\end{equation}
...
\begin{equation}
A_{celda} = \frac{A_{total}}{n_{celdas}} = \frac{41.1}{3} = 13.7 \text{ m}^2 \text{ por celda}
\end{equation}

Con una relacion largo/ancho de 3:1, las dimensiones de cada bloque son 6.4 m de largo por 2.1 m de ancho.
...
\begin{equation}
\rho_S = \frac{M_{SST} \times 365}{A_{total}} = \frac{16.42 \times 365}{41.1} = 146.0 \text{ kg SST/m}^2\text{·anio}
\end{equation}
...
\bottomrule
\endlastfoot
Area total requerida & 41.1 m$^2$ \\
Numero de bloques (corresponde al numero de lineas) & 3 \\
Area por bloque & 13.7 m$^2$ \\
Dimensiones de cada bloque (L$\times$A) & 6.4 m $\times$ 2.1 m \\
Numero total de celdas & 3 \\
Tiempo de secado & 15 dias \\
Carga de solidos & 146.0 kg SST/m$^2$·anio \\
Produccion total de lodos & 16.42 kg SST/d \\
\end{longtable}
\endgroup
...
El lecho de secado ha sido dimensionado para tratar 16.42 kg SST/d de lodos generados en el proceso de tratamiento, distribuidos en 3 bloques independientes (uno por linea de tratamiento). Cada bloque opera con un ciclo de 15 dias de secado.
...
\midrule
Zona de amortiguación & Perimetral alrededor de unidades (2--3~m) para acceso y seguridad & 376~m$^2$ (20\%) \\
\addlinespace
Bodega de químicos & Almacenamiento de hipoclorito y productos químicos & 23~m$^2$ \\
\addlinespace
Laboratorio/control & Análisis de calidad del agua (DBO, SST, CF, pH) & 34~m$^2$ \\
\addlinespace
Caseta de operación & Control, panel, escritorio, baño & 28~m$^2$ \\
\addlinespace
Área de lavado & Limpieza de equipos y herramientas & 19~m$^2$ \\
\addlinespace
Estacionamiento & Vehículos del personal y visitantes (2--3 vehículos) & 103~m$^2$ \\
\addlinespace
Zona de camiones & Acceso de camiones cisterna/retiro de lodo & 122~m$^2$ \\
\addlinespace
Caminos internas & Circulación vehicular y peatonal (ancho 3--4~m) & 188~m$^2$ (10\% área) \\
\addlinespace
Acceso principal & Entrada, portón y caseta de guardia & 47~m$^2$ \\
\addlinespace
Bodega general & Herramientas, repuestos, EPP & 34~m$^2$ \\
\addlinespace
Zona carga de lodos & Carga/descarga de lodo deshidratado & 47~m$^2$ \\
\addlinespace
Área verde/buffer & Separación de límites, revegetación & 481~m$^2$ (15\% total) \\
\bottomrule
\end{tabular}
\end{table}
...
Considerando el área de tratamiento (1879~m$^2$), amortiguación (20\%), complementarias operativas y zona verde (15\% del total):
...
\begin{itemize}[noitemsep,leftmargin=2em]
    \item[$A_{tratamiento}$] = 1879~m$^2$
    \item[$A_{amortiguación}$] = 376~m$^2$ (20\%)
    \item[$A_{complementarias}$] = 470~m$^2$ (25\% operativas estimado)
\end{itemize}

\begin{equation}
A_{total} = \frac{1879 + 376 + 470}{0.85} \approx \mathbf{3205 \text{ m}^2} \approx \mathbf{0.32 \text{ ha}}
\end{equation}

\textbf{Nota:} El área total del predio debe ser de aproximadamente 3205~m$^2$ (0.32~ha) para operación adecuada con circulación interna, estacionamiento y zonas verdes.
...
Reactor UASB & D = 8.25, H = 6.4 & 3 & v$_{up}$ = 0.39 m/h \\
TAF & D = 3.83, H = 4.0 & 3 & CHS = 3.61 m$^3$/m$^2$·h \\
...
Lecho de Secado & 6.4 $\times$ 2.1 & 3 & Área: 13.7 m$^2$ \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Area de tratamiento & 1879 m$^2$ \\
Area de amortiguacion (20\%) & 376 m$^2$ \\
Area complementaria operativa & 470 m$^2$ (25\% estimado) \\
Zona verde (15\% del total) & 481 m$^2$ \\
\midrule
\textbf{Area total requerida} & \textbf{3205 m$^2$ (0.32 ha)} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Reactor Anaerobio de Flujo Ascendente con Manto de Lodos (UASB) & 3.53 kg SST/d & 10.60 kg SST/d \\
Biofiltro de Carga Mecanizada e Hidráulica & 0.97 kg SST/d & 2.91 kg SST/d \\
Sedimentador Secundario & 0.97 kg SST/d & 2.91 kg SST/d \\
\midrule
\textbf{Total lodos} & \textbf{5.47 kg SST/d} & \textbf{16.42 kg SST/d} \\
Area de lechos de secado & --- & 41.1 m$^2$ (3 bloques) \\
